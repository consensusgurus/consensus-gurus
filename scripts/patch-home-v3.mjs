...
const circuitsOf = (g) => CIRCUITS.filter(([, names]) => names.includes(g.name)).map(([n]) => n);`,
  ...
  // The slate's own filter learns about circuits, which is what lets its real
  // rows answer an open circuit without a second row renderer existing.
  s = sub(s,
  ...
  s = sub(s,
    ...
    'DS:board-open', 1, ` + (cats && `);

  // cb-open has to mean ANY open filter, not only a category name, or a circuit
  // opens without the board switching out of grid mode. Its own edit: the one
  // above is an insert and is skipped once it is on origin.
  s = sub(s,
    `(cats && slateCats.includes(filter) ? ' cb-open' : '')`,
    `(cats && filter !== 'all' && filter !== 'todo' && filter !== 'sunday' ? ' cb-open' : '')`,
    'DS:board-open-any');
