...
# The whole zipf>=3.5 pool, not its 900 most frequent members. With no two-letter
# slots to absorb the awkward crossings, a 900-word cut left the 6x6 and 7x7
# templates almost unfillable (7x7 filled 10 times in 48 tries at 900, 40 at 2100).
CAP = 2100
...
# The 6x6/7x7/8x8 sets shipped before that date manufactured two-letter slots, and
# at length 2 the common-word filter below does nothing: 108 of the 124 two-letter
# words in tuck-dict.txt clear zipf 3.5, because two-letter strings are inflated by
# abbreviations (st 5.20, from Street/Saint) and by foreign text bleeding into an
# English corpus (ja 3.71, si 4.17, ka 3.83). So those slots filled with Scrabble
# two-letter words no reader accepts as words, and the grid was correct only by a
# dictionary nobody had been told about. Removing the slot removes the problem.
# Every template below is 180-degree symmetric, connected, and every across/down
# run is 3 to 7 letters (7 is the longest word the common pool carries). Machine
# searched by scripts/gen-shards/tsearch.py and kept only if it fills reliably;
# MIN_RUN below re-checks the whole set at import so a short run cannot creep back.
T5 = [
 ["##...","#....",".....","....#","...##"],
 ["...##","....#",".....","#....","##..."],
]
...
# A run of two is a two-letter slot, which is the whole defect above. Fail loudly at
# import rather than letting a bad template reach a bank.
MIN_RUN = 3
def _runs(pat, n):
    out = []
    for i in range(n):
        for line in (''.join(pat[i][j] for j in range(n)), ''.join(pat[j][i] for j in range(n))):
            out += [len(s) for s in line.split('#') if s]
    return out
for _n, _T in TEMPLATES.items():
    for _i, _p in enumerate(_T):
        _r = _runs(_p, _n)
        if min(_r) < MIN_RUN:
            raise AssertionError(f'template {_n}x{_n}[{_i}] has a run of {min(_r)}; minimum is {MIN_RUN}')
        if max(_r) > 7:
            raise AssertionError(f'template {_n}x{_n}[{_i}] has a run of {max(_r)}; the word pool tops out at 7')
