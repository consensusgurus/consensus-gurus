...
**3. ⚠️ THE MODELS RUN OUT LONG BEFORE THE BOARD DOES, and this is the open problem.** The snapshot
stores each analytics model's top 50, but F+ publishes 138, FPI 136 and Sagarin 266. A team below a
model's stored depth correctly takes that model's tail mean, so **80 of 138 teams shared ONE
identical analytics value** while `rankDesc` handed them ranks 59 through 138 off its index
tiebreak. The MODELS column printed a confident, ordered, entirely fabricated opinion for 58% of the
page under a header claiming the models carry 42%.

The guard now in place: a team no live model ranks is flagged (`modelSeen`), its MODELS cell reads
`off board` instead of a rank, it is excluded from the rank-range and spread, and a note computed
from the board says how deep the models actually reach this week. **This is a HONEST DISPLAY, not a
fix.** The fix is to store the model columns at their published depth, which needs a build session
that can reach `bcftoys.com`, `sagarin.com` and `dratings.com`. Until then, understand plainly that
**below the models' stored depth the order is the market's and the results', not the models'.**

Do not "restore" the model ranks by removing the `modelSeen` check. The numbers it suppresses are
generated by array order, not by any model.
...
- [ ] **Store the analytics models at their PUBLISHED depth, not the top 50** (§2d). This is the
      biggest open correctness item on the page: 80 of 138 teams currently read `off board` in the
      MODELS column. F+ publishes 138, FPI 136, Sagarin 266, DRatings all of FBS. Needs a build
      session with egress to `bcftoys.com`, `sagarin.com` and `dratings.com`.
