...
- **The Infatuation sources — order by their 0–10 score, never "unordered."** The Infatuation publishes a numeric review score (0–10, e.g. `8.4`) for nearly every restaurant, shown on each spot's page and in the guide imagery. Their roundup guides *look* unordered (the list order is editorial/geographic), but the scores ARE a ranking. So whenever an Infatuation guide is a source: pull each listed restaurant's individual 0–10 score (open the spot's review page or read it off the guide), and order the source by score, highest first. Do NOT flag an Infatuation source `"unordered": true` and do NOT trust the raw guide order. Note the basis in the label, e.g. `'The Infatuation Boston · Best Italian 2026 (by 0–10 score)'`. Ties: break by review recency or list order. If a spot genuinely has no published score, place it last or omit it. **The 0–10 score is very often rendered as an image/graphic in the article, NOT selectable text — so a plain `web_fetch` or `get_page_text` extraction will silently MISS it and the spot will look unscored.** When the score is absent from the extracted text, read it visually instead: in the connected Chrome, screenshot/zoom the score badge on the guide, or open the spot's individual review page (the big number near the top) and read it off the rendered page. Never conclude a spot is unscored from a text fetch alone — confirm visually first, and only treat it as unscored after the rendered page also shows none.
