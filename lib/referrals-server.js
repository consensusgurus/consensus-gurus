...
export async function topReferrers(admin, { days = 30, limit = 10 } = {}) {
  ...
  const { data: rows, error: rowErr } = await admin
    .from('quiz_referrals')
    .select('referrer_user_id, referred_key, referred_user_id, created_at')
    .gte('created_at', since)
    .limit(5000);
  if (rowErr || !rows) return [];

  // Engagement of each referred person = how many games they have finished, so
  // a referrer who brings in players who stick outranks one who brings in
  // one-and-done visitors. Two batched reads keyed by the referred browser's
  // anon id (and, once they register, their user id). Best-effort: if the
  // engagement reads fail, ties simply fall through to most-recent referral.
  const keys = [...new Set(rows.map((r) => r.referred_key).filter(Boolean))];
  const uids = [...new Set(rows.map((r) => r.referred_user_id).filter(Boolean))];
  const playsByKey = new Map();
  const playsByUid = new Map();
  try {
    if (keys.length) {
      const { data: gp } = await admin
        ...
        .in('anon_id', keys)
        .limit(50000);
      for (const g of gp || []) playsByKey.set(g.anon_id, (playsByKey.get(g.anon_id) || 0) + 1);
    }
    if (uids.length) {
      const { data: gu } = await admin
        ...
        .in('user_id', uids)
        .limit(50000);
      for (const g of gu || []) playsByUid.set(g.user_id, (playsByUid.get(g.user_id) || 0) + 1);
    }
  } catch { /* engagement is best-effort */ }

  const tally = new Map();
  for (const r of rows) {
    const cur = tally.get(r.referrer_user_id) || { credits: 0, plays: 0, last: r.created_at };
    cur.credits += 1;
    // Count each referred person's games once: prefer the registered-user count
    // when we have a user id, else the browser-key count.
    cur.plays += r.referred_user_id != null
      ? (playsByUid.get(r.referred_user_id) || 0)
      : (playsByKey.get(r.referred_key) || 0);
  ...
  return [...tally.entries()]
    .map(([id, v]) => ({ username: byId.get(id)?.username || null, refCode: byId.get(id)?.ref_code || null, credits: v.credits, plays: v.plays, last: v.last }))
    .filter((r) => r.username)
    .sort((a, b) => b.credits - a.credits || b.plays - a.plays || (a.last > b.last ? -1 : 1))
    .slice(0, limit)
