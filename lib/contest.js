...
export const CONTEST = {
  ...
  // --- prize ----------------------------------------------------------------
  // Weighted to first place (owner, 2026-08-05; was a flat $5 each). A single
  // headline number is a better hook than three small equal ones, and second
  // and third still pay, so the chase does not collapse once someone pulls
  // ahead. Index 0 is first place; `winners` is derived, never hardcoded.
  prizes: [20, 5, 5],
  ...
  // is what makes a worldwide $5 prize practical without asking a stranger for
  // an IBAN. The reader is shown the OPTIONS, never the rails.
  payoutRails: ['venmo', 'tremendous'],
  payoutOptions: ['Venmo', 'PayPal', 'bank transfer', 'gift card'],
...
export const COPY = {
  ...
  // "$20 first, $5 second and third" rather than a total: the reader wants to
  // know what THEY could win, not the size of the pot.
  prizeLine: `$${CONTEST.prizes[0]} first, $${CONTEST.prizes[1]} second and third`,
  ...
  payoutLine: 'Open worldwide. Winners pick how they are paid: Venmo, PayPal, bank transfer or gift card.',
  fraudLine:
