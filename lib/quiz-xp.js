...
// and the curve above is the whole story. Blocks is not: it is endless by
...
// exactly one game in the roster. It is capped at 1 instead (owner, 2026-08-08):
...
export const XP_DAILY_CAP = { blocks: 1 };
...
export function computeXp(rows, { recentN = 20, rankFor = null, nowMs = Date.now() } = {}) {
  ...
  for (const r of ordered) {
    ...
    if (isDailyGame(r.quiz_id)) {
      ...
      if (capped != null) gained = Math.min(gained, capped);
    }
