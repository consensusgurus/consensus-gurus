// Additive XP progression for quiz players, computed from the existing
...
// engagement metric should do. XP only ever goes UP.
//
// Per completed game:
...
//
// Levels: cumulative XP to REACH level L grows 25 XP per step,
...
// (an unattributable one-off play). Per-category XP uses the same per-game
// values restricted to that category's games; quizDept() supplies the category.

import { quizDept } from './quiz-departments.js';
import { correctAnswersOf, answeredOf, isDailyGame } from './quiz-scoring.js';
...
// A daily puzzle asks a single question ("did you solve it?"), so paying XP per
// correct answer earned a solved daily 1-2 XP against 50+ for a large
...
// Doubles as the XP multiplier: Dq / 1000 = 1.0× (easy) .. 2.0× (brutal).
...
// Cumulative XP needed to REACH level L (level 1 = 0 XP). The step from L to
...
export function xpTier(level) {
  ...
  if (L >= 18) return { label: 'Master Tier', bg: '#e8effb', fg: '#0e1d40' };
  if (L >= 14) return { label: 'Diamond Tier', bg: '#e8effb', fg: '#0e1d40' };
  if (L >= 9) return { label: 'Gold Tier', bg: '#f3e3c8', fg: '#8a5a00' };
  if (L >= 5) return { label: 'Silver Tier', bg: '#eceef1', fg: '#6b7280' };
...
//   xp, level, matches, xp7d (XP earned in the last 7 days; null = every game
...
//   recent: [{ quizId, dq, scorePct, xp, attempt, perfect, createdAt, rankDelta, catRankDelta }], // newest first
...
export function computeXp(rows, { recentN = 20, rankFor = null, nowMs = Date.now() } = {}) {
  ...
  // XP earned recently rather than all-time so the spot stays winnable.
  ...
  for (const r of ordered) {
    ...
    // Dailies earn the flat answer-equivalent above, pro-rated by solve share;
    // every other quiz earns per correct answer.
    const credit = isDailyGame(r.quiz_id)
      ? XP_DAILY_ANSWERS * (answered > 0 ? correct / answered : 0)
      : correct;
    ...
    // (rankFor). Rank = count of others currently with more XP; delta =
    // (#ahead before) − (#ahead after), so positive = moved UP. XP is
...
// XP desc, then correct answers desc, then accuracy desc, then handle.
export function rankPlayers(players, scope = 'all', { minMatches = 1 } = {}) {
  const list = [];
  for (const p of players.values()) {
    let xp, level, matches, correct, completed, daysPlayed, accuracy, played, xp7d, xp30d;
    if (scope === 'all') {
      xp = p.xp; level = p.level; matches = p.matches; xp7d = p.xp7d; xp30d = p.xp30d;
      ...
      if (!c) continue;
      xp = c.xp; level = c.level; matches = c.matches; xp7d = undefined; xp30d = undefined;
