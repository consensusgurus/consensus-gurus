...
// completion axis lib/daily-combined scores at 5 x score/total, so that fraction
// is the partial share when the solved flag is 0.
//
// The curve is deliberately CONVEX, not linear: a half-finished single puzzle
// has not actually solved anything, so half pays ONE answer-equivalent of the
// twelve (0.5 ^ XP_PARTIAL_CURVE = 1/12 by construction), a quarter pays ~0, and
// only a near-solve pays near-full. Worked shares of the 12:
//   1.00 -> 12    0.94 -> 9.5    0.75 -> 4.6    0.50 -> 1.0    0.25 -> 0.1
// This is also why a half-solved Links still pays six answer-equivalents while a
// half-filled Bracket pays one: Links reports a COUNT of finished sub-puzzles
// (two of its four groups genuinely solved, share 0.5 on the solved axis), so it
// never touches this curve. Bracket's half is one unsolved puzzle.
//
...
// Solved (or a count-based daily's solved fraction) wins; otherwise the curved
// points fraction. Never below the solved fraction, so nothing loses IQ Points.
function dailyShare(row, correct, answered) {
  ...
  const partial = pts > 0 ? Math.pow(pts, XP_PARTIAL_CURVE) : 0;
  return Math.max(solved, partial);
}
