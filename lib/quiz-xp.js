...
export function xpTier(level) {
  ...
  if (L >= 18) return { label: 'Master Tier', bg: '#e8effb', fg: '#2563eb' };
  if (L >= 14) return { label: 'Diamond Tier', bg: '#e8effb', fg: '#2563eb' };
...
export function rankPlayers(players, scope = 'all', { minMatches = 1 } = {}) {
  const list = [];
  for (const p of players.values()) {
    let xp, level, matches, correct, completed, daysPlayed, accuracy, played, xp7d;
    if (scope === 'all') {
      xp = p.xp; level = p.level; matches = p.matches; xp7d = p.xp7d;
      ...
      if (!c) continue;
      xp = c.xp; level = c.level; matches = c.matches; xp7d = undefined;
