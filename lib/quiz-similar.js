...
import { BN_NEWS_RE, isBusinessNewsHubQuiz } from './business-news-hub';
...
// A dated market/business news-recap quiz (daily-market-news, daily-/weekly-business,
// earnings-reporter). These belong to the Business News hub and are not topically
// "similar" to anything else, so they must never surface as a related / play-next
// pick for a non-business-news quiz.
function isNewsRecapQuiz(q) {
  return !!(q && q.id && BN_NEWS_RE.test(q.id));
}

// Whether `candidate` may be offered as a similar / play-next pick while the player
// is on `source`. News-recap quizzes are gated to business-news sources only; every
// other candidate is allowed (further ranking happens in the callers).
export function allowInSimilar(source, candidate) {
  if (!candidate || !candidate.id) return false;
  if (isBusinessNewsHubQuiz(source && source.id)) return true;
  return !isNewsRecapQuiz(candidate);
}

function pickNext(quiz, played) {
  if (!quiz || !quiz.id) return null;
  const seen = played instanceof Set ? played : new Set();
  const usable = (q) => q && q.id && q.id !== quiz.id && !q.hideFromRelated && allowInSimilar(quiz, q);
