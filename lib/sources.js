...
const BRAND_NAMES = {
  ...
  'superyachtfan.com': 'SuperYachtFan',
...
export function getAllSources() {
  ...
  for (const list of LISTS) {
    ...
    // For fact-based lists the single `ai` source IS the real publication
    // (Box Office Mojo, SuperYachtFan, RealTrends...), so surface it.
    const aiIsRealSource = list.mode === 'facts';
    for (const sid of Object.keys(src)) {
      if (EXCLUDE_IDS.has(sid) && !(sid === 'ai' && aiIsRealSource)) continue;
      const s = src[sid] || {};
      ...
      const key = domain || (sid === 'ai' ? `label:${cleanLabel(s.label)}` : `id:${sid}`);
