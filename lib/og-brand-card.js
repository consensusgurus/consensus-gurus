...
// ---------------------------------------------------------------------------
// Tables share card. The demo league is NEUTRAL (invented clubs and results),
// so it never spoils a real season. Ledger blue (#1d4ed8).
function tablesBoardEl() {
  const cell = (v, key) => {
  ...
  return h('div', { style: { display: 'flex', flexDirection: 'column', alignItems: 'flex-start', background: '#f4f2f5', border: '2px solid #1c1e24', borderRadius: 14, padding: '18px 20px 12px' } }, [
    h('div', { key: 'mh', style: { display: 'flex', flexDirection: 'row', alignItems: 'center', width: '100%', borderBottom: '3px solid #1c1e24', paddingBottom: 8, marginBottom: 10 } }, [
      ...
      h('div', { key: 'x', style: { display: 'flex', fontSize: 14, fontWeight: 800, letterSpacing: '1.5px', color: '#fff', background: '#1d4ed8', borderRadius: 5, padding: '3px 9px', marginLeft: 12 } }, 'TABLES'),
    ]),
...
function buildTablesCard() {
  ...
  return h('div', { style: { width: '100%', height: '100%', display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', background: '#f7f8fa', fontFamily: 'Manrope', padding: '54px 60px', position: 'relative' } }, [
    ...
    h('div', { key: 'left', style: { display: 'flex', flexDirection: 'column', width: '560px' } }, [
      ...
      h('div', { key: 'ttl', style: { display: 'flex', alignItems: 'center' } }, [
        T('Tables', { fontSize: 88, fontWeight: 800, letterSpacing: '-3px', color: '#1c1e24', lineHeight: 1 }),
      ...
      T('PLAY FREE · SOURCEOFTRUTHS.COM/TABLES', { fontSize: 20, fontWeight: 800, letterSpacing: '1.5px', color: '#1c1e24', marginTop: 28 }),
    ]),
    h('div', { key: 'right', style: { display: 'flex' } }, tablesBoardEl()),
  ]);
}
export async function renderTablesCard() {
  const [w8, w7, w6] = await Promise.all([woff(800), woff(700), woff(600)]);
  const any = w8 || w7 || w6;
  const loaded = { 800: w8, 700: w7, 600: w6 };
  const fonts = any ? [800, 700, 600].map((wt) => ({ name: 'Manrope', data: loaded[wt] || any, weight: wt, style: 'normal' })) : [];
  return new ImageResponse(buildTablesCard(), { ...size, fonts });
}
...
function buildBracketCard() {
  ...
  return h('div', { style: { width: '100%', height: '100%', display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', background: '#f7f8fa', fontFamily: 'Manrope', padding: '54px 56px', position: 'relative' } }, [
    ...
    h('div', { key: 'left', style: { display: 'flex', flexDirection: 'column', width: '540px' } }, [
      ...
      T('Bust your bracket.', { fontSize: 33, fontWeight: 800, color: '#c2410c', letterSpacing: '-0.5px' }),
