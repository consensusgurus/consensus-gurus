...
function taireBoardDataURI() {
  ...
  const card = (x, y, r, s) => {
    ...
    return `<g><rect x="${x}" y="${y}" width="${CW}" height="${CH}" rx="7" fill="#fdfcf9" stroke="rgba(20,22,28,0.30)"/>`
      + `<text x="${x + CW / 2}" y="${y + 30}" font-family="Manrope,sans-serif" font-size="20" font-weight="800" fill="${col}" text-anchor="middle">${TAIRE_LABEL[r]}</text>`
      + `<text x="${x + CW / 2}" y="${y + 54}" font-family="Manrope,sans-serif" font-size="19" fill="${col}" text-anchor="middle">${s === 1 ? '&#9829;' : '&#9824;'}</text></g>`;
  };
