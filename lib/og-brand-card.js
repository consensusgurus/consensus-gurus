...
// ---------------------------------------------------------------------------
// Scrab share card — snapshot of the game. The demo position is a NEUTRAL
...
function scrabTileEl(ch, pts, key, opts = {}) {
...
function scrabBoardEl() {
  // A five-square strip of board with a word already down and the premium
  // squares showing, plus the rack underneath. Enough to read as Scrabble
  ...
  return h('div', {
    ...
    h('div', { key: 'rack', style: { display: 'flex', flexDirection: 'row' } }, [
      scrabTileEl('E', 1, 't1'), scrabTileEl('R', 1, 't2'), scrabTileEl('V', 4, 't3'),
      scrabTileEl('O', 1, 't4'), scrabTileEl('D', 2, 't5', { gap: 0 }),
    ]),
  ]);
}

function buildScrabCard() {
  const T = (txt, style) => h('div', { style: { display: 'flex', ...style } }, txt);
  return h('div', { style: { width: '100%', height: '100%', display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', background: '#f7f8fa', fontFamily: 'Manrope', padding: '54px 60px', position: 'relative' } }, [
    ...
    h('div', { key: 'left', style: { display: 'flex', flexDirection: 'column', width: '560px' } }, [
      ...
      h('div', { key: 'ttl', style: { display: 'flex', alignItems: 'center' } }, [
        T('Scrab', { fontSize: 100, fontWeight: 800, letterSpacing: '-3px', color: '#1c1e24', lineHeight: 1 }),
      ...
      T('A Scrabble game picked up at the very end. Nothing is left to draw, so their rack is not a secret. Race them out, or make them sit on a tile they cannot play.', { fontSize: 24, fontWeight: 600, color: '#6b7280', lineHeight: 1.34, marginTop: 15, maxWidth: 545 }),
      T('PLAY FREE · SOURCEOFTRUTHS.COM/SCRAB', { fontSize: 20, fontWeight: 800, letterSpacing: '1.5px', color: '#1c1e24', marginTop: 30 }),
    ]),
    h('div', { key: 'right', style: { display: 'flex' } }, scrabBoardEl()),
  ]);
}

export async function renderScrabCard() {
  const [w8, w7, w6] = await Promise.all([woff(800), woff(700), woff(600)]);
  const any = w8 || w7 || w6;
  const loaded = { 800: w8, 700: w7, 600: w6 };
  const fonts = any ? [800, 700, 600].map((wt) => ({ name: 'Manrope', data: loaded[wt] || any, weight: wt, style: 'normal' })) : [];
  return new ImageResponse(buildScrabCard(), { ...size, fonts });
}
