...
function buildCard(count) {
  ...
  return h('div', { style: { width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#f7f8fa', fontFamily: 'Manrope', position: 'relative' } }, [
    ...
    T('EXERCISE YOUR MIND', { fontSize: 33, fontWeight: 700, letterSpacing: '8px', color: '#0e1d40' }),
    T('DAILY PUZZLES · QUIZZES · TOP 10 LISTS · SOURCEOFTRUTHS.COM', { fontSize: 19, fontWeight: 700, letterSpacing: '2.5px', color: '#9aa0ab', marginTop: '24px' }),
  ]);
}
...
// ---------------------------------------------------------------------------
// Bid share card. The demo sale is not one of the 28 in the bank.
const BID_DEMO = [
  { name: 'Grandfather clock', value: 30 },
  { name: 'Brass telescope', value: 25 },
  { name: 'Wingback chair', value: 20 },
  { name: 'Boxed china', value: 15 },
  { name: 'Crate of LPs', value: 10 },
];
function bidBoardEl() {
  const T = (txt, style) => h('div', { style: { display: 'flex', ...style } }, txt);
  return h('div', { style: { display: 'flex', flexDirection: 'column', background: '#fff', border: '2px solid #1c1e24', borderRadius: 14, padding: '22px 24px 18px', width: '430px' } }, [
    h('div', { key: 'hd', style: { display: 'flex', fontSize: 15, fontWeight: 800, letterSpacing: '2px', color: '#6b7280', marginBottom: 14 } }, 'PURSE: 100'),
    ...BID_DEMO.map((l, i) => h('div', {
      key: `l${i}`,
      style: { display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 },
    }, [
      T(l.name, { fontSize: 22, fontWeight: 700, color: '#1c1e24' }),
      h('div', { key: 'v', style: { display: 'flex', alignItems: 'center', justifyContent: 'center', minWidth: '52px', height: '38px', borderRadius: '8px', background: '#7c2d12', color: '#fff', fontSize: 20, fontWeight: 800 } }, String(l.value)),
    ])),
  ]);
}
function buildBidCard() {
  const T = (txt, style) => h('div', { style: { display: 'flex', ...style } }, txt);
  return h('div', { style: { width: '100%', height: '100%', display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', background: '#f7f8fa', fontFamily: 'Manrope', padding: '54px 60px', position: 'relative' } }, [
    h('div', { key: 'bar', style: { position: 'absolute', top: 0, left: 0, width: '1200px', height: '9px', display: 'flex', background: 'linear-gradient(90deg,#0a1730,#7c2d12 55%,#d9a184)' } }),
    h('div', { key: 'left', style: { display: 'flex', flexDirection: 'column', width: '560px' } }, [
      h('div', { key: 'brand', style: { display: 'flex', alignItems: 'center', marginBottom: 22 } }, [
        h('img', { key: 'i', src: iconRingsDataURI(), width: 72, height: 72, style: { marginLeft: '-10px', marginRight: '2px' } }),
        T('Source of Truths', { fontSize: 28, fontWeight: 800, letterSpacing: '-1px', color: '#1c1e24' }),
      ...
      h('div', { key: 'ttl', style: { display: 'flex', alignItems: 'center' } }, [
        T('Bid', { fontSize: 100, fontWeight: 800, letterSpacing: '-3px', color: '#1c1e24', lineHeight: 1 }),
      ...
      T('One purse. Five lots. No right answer.', { fontSize: 31, fontWeight: 800, color: '#7c2d12', letterSpacing: '-0.5px' }),
      T('The daily auction. Split a fixed purse across five lots and take the ones where you outbid the crowd. You cannot win everywhere, so the game is deciding where to walk away.', { fontSize: 24, fontWeight: 600, color: '#6b7280', lineHeight: 1.34, marginTop: 15, maxWidth: 540 }),
      T('PLAY FREE · SOURCEOFTRUTHS.COM/BID', { fontSize: 20, fontWeight: 800, letterSpacing: '1.5px', color: '#1c1e24', marginTop: 30 }),
    ]),
    h('div', { key: 'right', style: { display: 'flex' } }, bidBoardEl()),
  ]);
}
export async function renderBidCard() {
  const [w8, w7, w6] = await Promise.all([woff(800), woff(700), woff(600)]);
  const any = w8 || w7 || w6;
  const loaded = { 800: w8, 700: w7, 600: w6 };
  const fonts = any ? [800, 700, 600].map((wt) => ({ name: 'Manrope', data: loaded[wt] || any, weight: wt, style: 'normal' })) : [];
  return new ImageResponse(buildBidCard(), { ...size, fonts });
}
