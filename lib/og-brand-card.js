...
const CRUX_CATS = ['#e2795c', '#45a08c', '#6d9cc4', '#c76d95']; // coral teal steel plum
const CRUX_BOARD = {
  cols: 7,
  rows: 6,
  // key "r,c" -> { ch, cat? }. cat index into CRUX_CATS = "filed" tile.
  cells: {
    '0,1': { ch: 'S', cat: 1 }, '0,2': { ch: 'H', cat: 1 }, '0,3': { ch: 'O', cat: 1 }, '0,4': { ch: 'E', cat: 1 },
    '1,1': { ch: 'T' }, '1,3': { ch: 'P' },
    '2,1': { ch: 'E' }, '2,2': { ch: 'V' }, '2,3': { ch: 'E' }, '2,4': { ch: 'N' }, '2,5': { ch: 'L', cat: 2 }, '2,6': { ch: 'Y', cat: 0 },
    '3,1': { ch: 'A' }, '3,3': { ch: 'N' }, '3,5': { ch: 'E', cat: 2 }, '3,6': { ch: 'A', cat: 0 },
    '4,1': { ch: 'M', cat: 3 }, '4,2': { ch: 'A', cat: 3 }, '4,3': { ch: 'P', cat: 3 }, '4,5': { ch: 'A', cat: 2 }, '4,6': { ch: 'M', cat: 0 },
    '5,5': { ch: 'P', cat: 2 },
  },
};
  ...
  for (let r = 0; r < CRUX_BOARD.rows; r++) {
    ...
    for (let c = 0; c < CRUX_BOARD.cols; c++) {
      ...
      if (!cell) {
        ...
        cells.push(h('div', { key: c, style: { ...base, background: CRUX_CATS[cell.cat], color: '#fff', fontSize: 34, fontWeight: 800 } }, cell.ch));
      } else {
...
function buildCruxCard() {
  ...
  return h('div', { style: { width: '100%', height: '100%', display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', background: '#f7f8fa', fontFamily: 'Manrope', padding: '54px 60px', position: 'relative' } }, [
    ...
    h('div', { key: 'left', style: { display: 'flex', flexDirection: 'column', width: '560px' } }, [
      ...
      h('div', { key: 'legend', style: { display: 'flex', alignItems: 'center', marginTop: 24 } }, [pill('#e2795c'), pill('#45a08c'), pill('#6d9cc4'), pill('#c76d95')]),
