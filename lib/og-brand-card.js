...
function tallyBoardEl() {
  ...
  const cellEl = (r, c) => {
    const key = `${r},${c}`;
    if (blocked.has(key)) return h('div', { key, style: { display: 'flex', width: CELL, height: CELL, borderRadius: 10, background: '#1c1e24' } });
    const v = demo[r][c];
    return h('div', { key, style: { display: 'flex', alignItems: 'center', justifyContent: 'center', width: CELL, height: CELL, borderRadius: 10, background: '#fff', border: '2px solid rgba(28,30,36,0.45)', fontSize: 32, fontWeight: 800, color: '#1c1e24' } }, v ? String(v) : '');
  };
  const rowEl = (r) => h('div', { key: `r${r}`, style: { display: 'flex', flexDirection: 'row', alignItems: 'center', marginBottom: GAP } }, [
    ...Array.from({ length: N }, (_, c) => cellEl(r, c)),
    h('div', { key: 'sp', style: { display: 'flex', width: GAP } }),
    h('div', { key: 't', style: { display: 'flex', alignItems: 'center', justifyContent: 'center', width: 52, height: CELL, borderRadius: 14, background: '#15803d', color: '#fff', fontSize: 24, fontWeight: 800 } }, String(rowT[r])),
  ]);
  const colT = [10, 12, 14, 19];
  return h('div', { style: { display: 'flex', flexDirection: 'column', alignItems: 'flex-start', background: '#fff', border: '2px solid #1c1e24', borderRadius: 14, padding: '24px 26px 18px' } }, [
    ...
    h('div', { key: 'ct', style: { display: 'flex', flexDirection: 'row' } }, [
      ...colT.map((v, c) => h('div', { key: `c${c}`, style: { display: 'flex', alignItems: 'center', justifyContent: 'center', width: CELL, height: 40, marginRight: GAP, borderRadius: 12, background: '#15803d', color: '#fff', fontSize: 20, fontWeight: 800 } }, String(v))),
    ]),
  ]);
}
