...
// ---------------------------------------------------------------------------
// Park share card — snapshot of the game. The demo lot is hand-made and is NOT
// in the bank, so it never spoils today. Evergreen. Drawn as one inline SVG
...
function parkBoardDataURI() {
  ...
  PARK_DEMO.forEach(([len, horiz, fixed, pos], i) => {
...
function parkBoardEl() {
  ...
  return h('div', { style: { display: 'flex', flexDirection: 'column', alignItems: 'flex-start', background: '#fff', border: '2px solid #1c1e24', borderRadius: 14, padding: '20px 20px 18px' } }, [
    ...
    h('img', { key: 'bd', src: parkBoardDataURI(), width: BOX, height: BOX, style: { display: 'flex' } }),
  ]);
}
function buildParkCard() {
  const T = (txt, style) => h('div', { style: { display: 'flex', ...style } }, txt);
  return h('div', { style: { width: '100%', height: '100%', display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', background: '#f7f8fa', fontFamily: 'Manrope', padding: '54px 60px', position: 'relative' } }, [
    ...
    h('div', { key: 'left', style: { display: 'flex', flexDirection: 'column', width: '545px' } }, [
      ...
      h('div', { key: 'ttl', style: { display: 'flex', alignItems: 'center' } }, [
        T('Park', { fontSize: 100, fontWeight: 800, letterSpacing: '-3px', color: '#1c1e24', lineHeight: 1 }),
      ...
      T('Par is the proven minimum.', { fontSize: 33, fontWeight: 800, color: '#7c5c2e', letterSpacing: '-0.5px' }),
      T('The daily sliding-block jam. Every block slides on one axis, the red one has to reach the gap, and par is the true fewest moves, found by search rather than by hand. No undo, only a restart.', { fontSize: 24, fontWeight: 600, color: '#6b7280', lineHeight: 1.34, marginTop: 15, maxWidth: 530 }),
      T('PLAY FREE · SOURCEOFTRUTHS.COM/PARK', { fontSize: 20, fontWeight: 800, letterSpacing: '1.5px', color: '#1c1e24', marginTop: 30 }),
    ]),
    h('div', { key: 'right', style: { display: 'flex' } }, parkBoardEl()),
  ]);
}
export async function renderParkCard() {
  const [w8, w7, w6] = await Promise.all([woff(800), woff(700), woff(600)]);
  const any = w8 || w7 || w6;
  const loaded = { 800: w8, 700: w7, 600: w6 };
  const fonts = any ? [800, 700, 600].map((wt) => ({ name: 'Manrope', data: loaded[wt] || any, weight: wt, style: 'normal' })) : [];
  return new ImageResponse(buildParkCard(), { ...size, fonts });
}
