...
//
// EXCLUSIVE, BUT NOT EXHAUSTIVE (owner ruling, 2026-08-24). No game sits in two
// skill circuits, which is what stops one play paying into two skill boards at
// once. A game does NOT have to sit in one: where the only circuit that would
// take it is one whose title does not describe it, it stays out, and
// scripts/verify-circuits.mjs reports the uncovered games as a WARNING rather
// than a failure. Three quiz games (Blitz, Streak, Deep) are in none, for the
// reason set out in the Arcade note below. The count comes out at 66 of 69
// eligible games over 15 circuits (the Sudoku circuit holding EIGHT as a
// rotating pool that plays five a day - see its entry), EIGHT OF FIVE, FOUR OF
// FOUR, ONE OF THREE AND TWO PAIRS (Niche made Recall the eighth five on
// 2026-08-20, Shoe made Table Games the ninth on 2026-08-21, the chess split of
// 2026-08-21 left Board Games and Chess a pair of threes, Race's deletion later
// the same day took Board Games down to a pair, and Arcade became the second
// pair on 2026-08-24). The no-tiny-circuits floor still holds in general, a
...
// does not belong was the worse lie, sanctioned the pair when Race went,
// because Table Games was already at the cap of five with no room to take
// Check and Turn in, and sanctioned the Arcade pair on 2026-08-24 for the same
// reason in its strongest form: a five assembled out of three games the title
// does not describe is worth less than an honest pair.
...
export const CIRCUITS = [
  ...
  },
  {
    ...
    // describe the title. They are in no circuit now, which the
    // exclusive-not-exhaustive rule above allows. 437s still tiers silver, so
    // the trophy is unchanged, and the pair is above the floor of two.
