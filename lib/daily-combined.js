...
import { DAILY_KEYS, dailyUnitOfQuizId } from './daily-games';
export { DAILY_KEYS };
...
export function scoreGame(rows) {
  // One row per traceable player (registered or anonymous guest). A COMPLETED
  // attempt beats an abandoned one (a real finish supersedes an earlier abandon); within the same
  ...
  for (const r of (rows || [])) {
    if (!r) continue;
    if (!r.user_id && !r.anon_id) continue; // skip untraceable rows
  ...
  // run that also never cleared a row (owner, 2026-08-08). Every row here
  // belongs to one puzzle, so the unit is read once, not per comparison.
  const tally = entries.length ? !!dailyUnitOfQuizId(entries[0].quiz_id) : false;
