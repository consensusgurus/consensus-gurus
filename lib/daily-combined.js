...
//
// The problem: the eleven daily games (crux, emcee, garble, links, span, dating, tally,
...
// A player's DAILY total is the sum of their BEST FIVE game scores (best 5 of the day's slate),
...
// The eleven daily game keys, in the display order used on the /daily hub and the
// end-screen cross-promo. Keep in sync with the daily registries (see the
// daily-game-registries note): crux, garble, links, span, dating, tally, suds,
// circa, extra, carve. `closer` is NOT a daily and is intentionally absent.
export const DAILY_KEYS = ['crux', 'emcee', 'garble', 'links', 'span', 'dating', 'tally', 'suds', 'circa', 'extra', 'carve'];
...
export function scoreGame(rows) {
  // First attempt per registered user (lowest row id wins), guests dropped.
  const firstByUser = new Map();
  for (const r of (rows || [])) {
    if (!r || !r.user_id) continue;
    const k = `u:${r.user_id}`;
    const prev = firstByUser.get(k);
    if (!prev || (r.id || 0) < (prev.id || 0)) firstByUser.set(k, r);
  }
