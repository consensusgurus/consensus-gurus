...
// warning about.
//
// `lo`/`hi` are the 1-based positions the player's TIE GROUP spans (lo === hi
// when nothing ties). A tie is paid the MEAN OF THE RUNGS it spans, never the
// rung at the mean position: two tied for 1st share (15+12)/2 = 13.5, three tied
// for 1st share (15+12+10)/3 = 12.33. `field` and `ratio` are read only on the
// pre-cutover path, where they reproduce the old math exactly.
export function gamePoints(id, { lo, hi = lo, field = 0, ratio = 0 }) {
  if (usesLadder(id)) {
    let sum = 0;
    for (let pos = lo; pos <= hi; pos++) sum += ladderAt(pos);
    const placement = sum / (hi - lo + 1);
...
import { DAILY_KEYS, dailyUnitOfQuizId, isArcadeQuizId, isEndGameQuizId, endGamePlan } from './daily-games';
export { DAILY_KEYS };
...
export function scoreGame(rows) {
  ...
  const endgame = all.length ? isEndGameQuizId(all[0].quiz_id) : false;
  const egPlan = endgame ? endGamePlan(all) : null;
  ...
  // behind it. Attempts separate the solvers and the drawers; the unsolved fall
  // through to the same depth term every other board uses, because ranking them
  // on attempts would put giving up once above fighting through five. MUST stay
  // identical to egRank in lib/quiz-anon buildLeaderboard.
  const egRank = (a, b) => {
    const A = egOf(a), B = egOf(b);
    if (!A || !B) return 0;
    return A.tier - B.tier
      || (A.tier < 2 ? A.tries - B.tries : depth(a, b))
      || ((a.time_elapsed ?? 0) - (b.time_elapsed ?? 0));
  };
  const ranks = (a, b) => (endgame ? egRank(a, b) : (b.score - a.score
  ...
  while (i < N) {
    ...
    for (let k = i; k < j; k++) {
      ...
      const { completion, placement, points } = gamePoints(quizId, { lo: i + 1, hi: j, field: N, ratio });
      players.set(uk, {
        userKey: uk,
        username: r.username,
...
export function guestGameResult(guestRow, game, eg = null) {
  ...
  for (const p of regs) {
    ...
    if (eg) {
      ...
      const sooner = pr < eg.tries || (pr === eg.tries && pT < gT);
      if (pt < eg.tier || (pt === eg.tier && (eg.tier < 2 ? sooner : deeper))) better++;
    } else if (p.score > score || (p.score === score && aheadOnDepth(pP, pG, pT))) better++;
  }
  const rank = better + 1; // where the guest would sit in this game's field
  const { points } = gamePoints(game.quizId, { lo: rank, field: newN, ratio });
  return { points, rank, field: newN };
}
