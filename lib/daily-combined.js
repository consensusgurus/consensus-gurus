...
import { DAILY_KEYS, dailyUnitOfQuizId, isArcadeQuizId } from './daily-games';
export { DAILY_KEYS };
...
export function scoreGame(rows) {
  ...
  const perfKey = (r) => `${r.score}|${r.progress ?? ''}|${r.guesses_used ?? ''}|${r.time_elapsed ?? ''}`;
...
export function guestGameResult(guestRow, game) {
  ...
  for (const p of regs) {
    ...
    if (p.score > score || (p.score === score && aheadOnDepth(pP, pG, pT))) better++;
  }
