...
function scoreFraction(row) {
  ...
  if (total <= 0) return 0;
  return clamp((Number(row.score) || 0) / total, 0, 1);
}
...
export function computeElo(rows, { recentN = 20 } = {}) {
  ...
  for (const r of ordered) {
    ...
    if (!cr) { cr = { rating: ELO_START, matches: 0, netDelta: 0 }; p.byCategory[cat] = cr; }
    ...
    p.correct += Number(r.score) || 0;
    p.answered += Number(r.total) || 0;
    p.playedSet.add(r.quiz_id);
    if (Number(r.total) > 0 && Number(r.score) === Number(r.total)) p.perfectSet.add(r.quiz_id);
  ...
  for (const p of players.values()) {
    ...
    for (const cat of Object.keys(p.byCategory)) {
      p.byCategory[cat].rating = Math.round(p.byCategory[cat].rating);
      p.byCategory[cat].netDelta = Math.round(p.byCategory[cat].netDelta);
    }
...
export function rankPlayers(players, scope = 'all', { minMatches = 1 } = {}) {
  ...
  for (const p of players.values()) {
    let rating, matches;
    if (scope === 'all') { rating = p.rating; matches = p.matches; }
    else { const c = p.byCategory[scope]; if (!c) continue; rating = c.rating; matches = c.matches; }
