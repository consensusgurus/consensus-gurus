...
// Replays of the SAME quiz move your rating far less than the first play, so
// grinding one quiz (or repeatedly acing it) can't inflate your skill rating.
// The first attempt at a given quiz uses the full K; every later attempt of
// that same quiz uses K * ELO_REPLAY_FACTOR.
export const ELO_REPLAY_FACTOR = 0.1;

// Inactivity decay (REVERSIBLE): a player's rating drifts back toward ELO_START
// the longer since their last game, reaching baseline at ELO_DECAY_DAYS. It is
// applied as a DISTINCT final step (applyInactivityDecay) gated by
// ELO_DECAY_ENABLED, so setting that flag to false restores the raw, undecayed
// ratings with no other change.
export const ELO_DECAY_ENABLED = true;
export const ELO_DECAY_DAYS = 90; // ~3 months from last play to full baseline
...
//   accuracy(percent),
// }
export function computeElo(rows, { recentN = 20, rankFor = null, decay = true, nowMs = Date.now() } = {}) {
  ...
  for (const r of ordered) {
    ...
    // First play of a quiz moves the rating at full K; later replays of the
    // same quiz move it at ELO_REPLAY_FACTOR of K (see ELO_REPLAY_FACTOR).
    const attKey = p.key + '::' + r.quiz_id;
    const attemptNo = (attemptSeen.get(attKey) || 0) + 1;
    attemptSeen.set(attKey, attemptNo);
    const kEff = attemptNo === 1 ? ELO_K : ELO_K * ELO_REPLAY_FACTOR;
    const E = 1 / (1 + Math.pow(10, (dq - p.rating) / 400));
    let delta = kEff * (S - E);
    if (S >= 1) delta = Math.max(0, delta);
    p.rating += delta;
    p.netDelta += delta;
    p.matches += 1;
    ...
    // (rankFor) so the leaderboard route pays nothing. Rank = count of others
    // currently rated higher; delta = (#better before) - (#better after), so a
    // positive value means this game moved the player UP. Among players active
    // by this point in the replay (matches the leaderboard's played-only set).
    let rankDelta = null, catRankDelta = null;
    if (rankFor && p.key === rankFor && p.matches > 1) {
      const after = p.rating, before = p.rating - delta;
      let gtA = 0, gtB = 0;
      for (const q of players.values()) {
        if (q === p) continue;
        if (q.rating > after) gtA++;
        if (q.rating > before) gtB++;
      }
      rankDelta = gtB - gtA;
      const ca = cr.rating, cb = cr.rating - dc;
      let cgA = 0, cgB = 0;
      for (const q of players.values()) {
        if (q === p) continue;
  ...
  for (const p of players.values()) {
    ...
    // Distinct quiz ids this player has PLAYED (any attempt) and fully COMPLETED
    // (scored a perfect 100% at least once). Used for the per-quiz completion
    // indicators in the browse lists. Captured before the sets are deleted.
  ...
  if (ELO_DECAY_ENABLED && decay) applyInactivityDecay(players, nowMs);

  return { players, difficulty };
}

function inactivityFactor(msSinceLastPlay) {
  const days = (Number(msSinceLastPlay) || 0) / 86400000;
  return clamp(1 - days / ELO_DECAY_DAYS, 0, 1);
}

// Decay every player's overall + per-category rating toward ELO_START by time
// since last play. Stores the pre-decay value in ratingRaw and the applied
// change in decayDelta (<= 0) so the rating breakdown still reconciles.
export function applyInactivityDecay(players, nowMs = Date.now()) {
  for (const p of players.values()) {
    const f = inactivityFactor(nowMs - (p.lastPlayedMs || nowMs));
    p.ratingRaw = p.rating;
    p.rating = Math.round(ELO_START + (p.rating - ELO_START) * f);
    p.decayDelta = p.rating - p.ratingRaw;
    for (const cat of Object.keys(p.byCategory)) {
      const cr = p.byCategory[cat];
      const cf = inactivityFactor(nowMs - (cr.lastPlayedMs || nowMs));
      cr.ratingRaw = cr.rating;
      cr.rating = Math.round(ELO_START + (cr.rating - ELO_START) * cf);
    }
  }
}
...
export function rankPlayers(players, scope = 'all', { minMatches = 1 } = {}) {
  const list = [];
  for (const p of players.values()) {
    let rating, matches, correct, completed, daysPlayed, accuracy, played;
    if (scope === 'all') {
