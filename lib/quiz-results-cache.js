...
const COLS_FULL = 'id, user_id, username, quiz_id, score, total, correct_count, anon_id, created_at, time_elapsed, is_mobile, guesses_used';
// Progressive fallbacks for a DB that predates migrations 33 / 25 / 24, in the
// same spirit as the old per-route fallbacks: never let a deploy depend on
// migration timing.
const COL_TIERS = [
  COLS_FULL,
  COLS_FULL.replace(', guesses_used', ''),
  COLS_FULL.replace(', guesses_used', '').replace(', is_mobile', ''),
  COLS_FULL.replace(', guesses_used', '').replace(', is_mobile', '').replace(', correct_count', ''),
];
...
async function fullLoad(admin) {
  ...
  for (; colsIdx < COL_TIERS.length; colsIdx++) {
    const r = await fetchAllRows(admin, 'quiz_results', COL_TIERS[colsIdx], ['id']);
...
export async function loadQuizResultsCached(admin) {
  if (cache && Date.now() - cache.checkedAt < BURST_TTL_MS) {
    return { data: cache.rows, error: null };
  }
  if (!inflight) {
