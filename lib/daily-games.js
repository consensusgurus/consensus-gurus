...
const RAW = [
  ...
  { key: 'chomp', keepsAnswer: true, fastRetry: true, miss: 'Moves', name: 'Chomp', cat: 'Logic', tag: 'Eat them in order', how: 'A cast of mascots to eat in order, and every square you touch stays yours for the rest of the run. Nothing chases you and nothing is on a clock. The only things in your way are where you have already been and a few bolted-down bleachers, and the board is tight enough that finishing takes most of it.', color: '#a8430f', colorNavy: '#f0a071' },
...
export function wantsFastRetry(key) {
  return isEndGame(key) || isArcade(key) || !!(DAILY_GAME_MAP[key] || {}).fastRetry;
}
