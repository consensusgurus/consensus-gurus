...
// scoring.
const RAW = [
  { key: 'crux', miss: 'Guesses', name: 'Crux', cat: 'Word', tag: 'A clueless crossword', how: 'Eight words interlock in an empty grid, and four category hints are the only clues you get.', color: '#1e212c', colorNavy: '#5b9bff' },
  ...
  { key: 'stands', miss: 'Rejects', name: 'Stands', cat: 'Logic', tag: 'Rebuild the results', how: 'Everyone played everyone once, so rebuild the full results table from the few surviving facts.', color: '#a83f28', colorNavy: '#f0a996' },
  ...
  { key: 'lode', miss: 'Words', name: 'Lode', cat: 'Word', tag: 'Seven letters, rare words pay', how: 'Build words from seven letters around the core letter, and the rarer the word the bigger the pay.', color: '#8a6410', colorNavy: '#e0b13f' },
  ...
  { key: 'four', keepsAnswer: true, miss: 'Tries', name: 'Four', cat: 'End Game', tag: 'One column wins', how: 'The position is already won for you. Find the single column that keeps it, because a wrong drop is not taken back.', color: '#3a4152', colorNavy: '#9db8ff' },
  ...
  // override below.
  { key: 'park', keepsAnswer: true, miss: 'Moves', name: 'Parker', href: '/parker', cat: 'Logic', tag: 'Get the red one out', how: 'Everybody has blocked you in, every block is stuck on one axis, and there is one gap in the wall.', color: '#7c5c2e', colorNavy: '#f0cf9a' },
  ...
  { key: 'rung', keepsAnswer: true, miss: 'Rungs', name: 'Rung', cat: 'Word', tag: 'One letter at a time', how: 'Climb from one five-letter word to another, changing a single letter a rung, in as few rungs as you can.', color: '#155e75', colorNavy: '#7fd4e8' },
  { key: 'taire', keepsAnswer: true, miss: 'Moves', name: 'Taire', cat: 'Cards', tag: 'The daily solitaire', how: 'Twenty cards face up, two suits, and a perfect line that is the proven minimum, so nobody beats it.', color: '#1d6b4f', colorNavy: '#86efac' },
  ...
  { key: 'chomp', keepsAnswer: true, miss: 'Moves', name: 'Chomp', cat: 'Logic', tag: 'Eat them in order', how: 'A cast of mascots to eat in order, and every square you touch stays yours for the rest of the run. Nothing chases you and nothing is on a clock. The only things in your way are where you have already been and a few bolted-down bleachers, and the board is tight enough that finishing takes most of it.', color: '#a8430f', colorNavy: '#f0a071' },
  ...
  { key: 'blocks', miss: 'Shapes', unit: 'rows', name: 'Blocks', cat: 'Arcade', tag: 'Same shapes, same order', how: 'Falling shapes and the same order for everybody. Play as many runs as you like and your best one takes the board. It never speeds up, so a run ends on a hole you left three shapes ago.', color: '#a83f28', colorNavy: '#93b4f0' },
  ...
  { key: 'barter', miss: 'Over', name: 'Barter', cat: 'Word', tag: 'Trade the letters home', how: 'Every letter the answer needs is already on the board, scrambled. Trade two tiles at a time until all six words read true, on a budget of the proven minimum plus five.', color: '#be123c', colorNavy: '#fb7fa2' },
  { key: 'sixes', miss: null, name: 'Sixes', cat: 'Numbers', tag: 'The daily mini sudoku', how: 'A 6x6 sudoku in six boxes two squares tall and three wide. Every row, column and box holds 1 to 6 exactly once. The short one: about two minutes, nothing counts against you, and the clock decides the day.', color: '#a83f28', colorNavy: '#7da2f5' },
...
// all 56 clients: these eleven can reach the end of a run without the player ever
...
// route. Babel prints the benchmark and never the line.
//
...
// A single row's verdict. total is 10 on every End Game game, and a row written
// before `total` existed would read 0, so the guard keeps such a row out of the
// solved tier rather than crediting a win it cannot prove.
function endGameTier(r) {
...
export function endGamePlan(rows) {
  ...
  for (const list of byUser.values()) {
    ...
    for (let i = 0; i < list.length; i++) {
      const tier = endGameTier(list[i]);
      info.set(list[i], { tier, tries: i + 1 });
      if (tier === 0 && win < 0) win = i;
      if (tier === 1 && draw < 0) draw = i;
    }
    // The winning run represents the player, whichever attempt it landed on. A
    // draw only represents them when they never went on to win, so a player who
    // drew on try 1 and won on try 3 is a 3-try solver, not a drawer.
    let pick = win >= 0 ? win : draw;
    if (pick < 0) {
      // Never finished: their record is the run that got FURTHEST, not their
      // first. Replays count for solvers here, so they count for everyone.
      pick = 0;
      for (let i = 1; i < list.length; i++) {
        const a = list[i], b = list[pick];
        const pa = Number.isFinite(a.progress) ? a.progress : -1;
        const pb = Number.isFinite(b.progress) ? b.progress : -1;
        if (pa > pb || (pa === pb && (a.time_elapsed ?? Infinity) < (b.time_elapsed ?? Infinity))) pick = i;
      }
    }
