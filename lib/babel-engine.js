...
// script (scripts/gen-babel.mjs) computes each day's BENCHMARK with it, and the
...
// banked benchmark is not an estimate — it is exactly the best a player can force
// against the defence they will actually face. scripts/verify-babel.mjs
// recomputes every benchmark from the banked position and fails on any drift.
//
// The board is 11x11 (a 15x15 is unreadable on a phone and the endgame is a
// local fight anyway), the bag is 65 tiles in near-Scrabble proportions with
// the Q in and no blanks, and the word list is public/tuck-dict.txt — the same
...
// 65 tiles, every letter in near-Scrabble proportion for an 11x11 board.
//
...
// approximation to apologise for: the benchmark is DEFINED as the value of the game under
...
// player who finds a line the engine never considered simply beats the benchmark, which
...
// browser calls it on every reply, and the bank's benchmark is measured against it,
...
// ─── benchmark ───────────────────────────────────────────────────────────────────
// The benchmark is the spread our solver ACHIEVES from the player's seat against
// that
...
// derived from one search policy and then defended by a different one is a target
// nobody can hit, and that is precisely the bug this replaced. Defining it as
