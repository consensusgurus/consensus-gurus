...
// hand-written set of canonical buckets (display label + alias keys, exactly
// the quiz keyHit convention: substring either way, or every word of a
// multi-word key present). Anything that matches no canonical bucket forms a
// dynamic bucket keyed by its normalized string, so two players typing the
// same oddball answer still find each other.
//
...
const STOP_LEAD = new Set(['the', 'a', 'an', 'my', 'your', 'their', 'his', 'her', 'our', 'its', 'some', 'to', 'i']);

// Normalize one free-text answer: lowercase, strip accents + punctuation,
// collapse spaces, drop leading articles, light plural-strip on the last word.
export function normAnswer(s) {
  ...
  if (!t) return '';
  const words = t.split(' ');
  while (words.length > 1 && STOP_LEAD.has(words[0])) words.shift();
  const last = words[words.length - 1];
  if (last.length > 3 && last.endsWith('s') && !last.endsWith('ss') && !last.endsWith('us') && !last.endsWith('is')) {
    words[words.length - 1] = last.slice(0, -1);
  }
  return words.join(' ');
}

// Quiz-style key hit: substring either way (guess side needs 3+ chars when it
// is the shorter side), or every word of a multi-word key present in the guess.
function keyHit(guessNorm, keyNorm) {
  if (!guessNorm || !keyNorm) return false;
  if (guessNorm.indexOf(keyNorm) !== -1) return true;
  if (keyNorm.indexOf(guessNorm) !== -1 && guessNorm.length >= 3) return true;
  const kw = keyNorm.split(' ');
  if (kw.length >= 2) {
    const gt = new Set(guessNorm.split(' '));
    let all = true;
    for (const w of kw) if (!gt.has(w)) { all = false; break; }
    if (all) return true;
  }
  return false;
}

// Per-prompt matcher with the alias keys pre-normalized once.
function promptMatcher(prompt) {
  const buckets = (prompt.answers || []).map((b, i) => ({
    id: 'c' + i,
    label: b.c,
    keys: (b.k || []).map(normAnswer).filter(Boolean),
  }));
  return {
    buckets,
    // The bucket id for one raw answer, or null for an empty/unusable answer.
    bucketOf(raw) {
      const g = normAnswer(raw);
      if (!g) return null;
      for (const b of buckets) {
        for (const k of b.keys) if (keyHit(g, k)) return b.id;
      }
      return 'x:' + g;
    },
  };
