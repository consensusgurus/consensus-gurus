...
//
// Every banked board on Unpark, Rung and Taire carries the exact solver
...
// Axiom counts tests rather than moves: perfect is one, two or three out of a
// six test budget, so a scaled cushion would swallow the whole budget. A single
// test of slack is the entire cushion there.
export function parForTests(perfect, budget) {
  const p = Number(perfect) || 0;
  const b = Number(budget) || 0;
  return b ? Math.min(p + 1, b) : p + 1;
}
