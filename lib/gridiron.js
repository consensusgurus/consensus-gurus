...
//
// MODELS OUTWEIGH HUMAN POLLS (owner rule, 2026-08-28). Voters anchor on
// preseason expectation and brand, and they move slowly: a poll will not drop a
// name team as fast as the results say it should, and it rewards teams that
// were ranked highly to begin with. The models re-derive from results every
// week with no such memory. Same instinct as the media-list crowd tilt already
// in CLAUDE.md, applied to the other side of the critic/objective divide.
const TIERS = {
  cfb: { official: 0.25, model: 0.45, media: 0.10, market: 0.20 },
  nfl: { official: 0.00, model: 0.40, media: 0.25, market: 0.35 },
};
...
export function computeComposite(sources, sport, opts = {}) {
  ...
  const solo = [...present].filter(
    (t) => live.filter(([, s]) => s.tier === t).length === 1 && tierShare[t] > SOLO_CAP);
  if (solo.length) {
    let freed = 0;
