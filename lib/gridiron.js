...
export const PARAMS = {
  ...
  cfb: { cap: 28, lam: 1, hbt: 0.30, mix: 0.6, tau: 0.5, tau0: 0.05, window: [1, 0.6, 0.35], linesShare: 0.75, hf: 2.5 },
  nfl: { cap: 21, lam: 1, hbt: 0.25, mix: 0.6, tau: 0.5, tau0: 0.05, window: [1, 0.6, 0.35], linesShare: 0.75, hf: 2.0 },
};
...
export function computeComposite(block, sport, opts = {}) {
  ...
  if (rrows.length) {
    ...
    const { x: r, H } = ridgeLS(
      rrows.map((g) => ({
        ...
        y: Math.max(-P.cap, Math.min(P.cap, g.m)) - (fitH ? 0 : P.hf * g.site),
      })),
      n, { lam: P.lam, fitH },
    );
    Hr = fitH ? H : P.hf;
    ...
    const k = sd(r) / (sd(b) || 1);
    R = r.map((v, i) => P.mix * v + (1 - P.mix) * k * b[i]);
  ...
  let M = new Array(n).fill(0), Hm = null;
  ...
  for (const w of weeks) {
    ...
    const first = Hm == null;
    const { x } = ridgeLS(rows, n, { lam: 0, fitH: false, tether: first ? P.tau0 : P.tau, prev: first ? null : M });
    M = x; Hm = P.hf;
    // With fewer than three weeks of lines the graph is barely connected and
    // the fit's SCALE is not identified (one line per team fixes a pair's
    // difference and nothing else), only its order. Until then the ratings
    // are rescaled so their spread matches what the lines themselves imply:
    // a spread is the difference of two ratings, so sd(rating) = sd(line)/sqrt2.
    if (weeks.indexOf(w) < 2) {
      const inRows = new Set(rows.flatMap((r) => [r.h, r.a]));
      const ix = [...inRows].filter((i) => i !== FCS);
  ...
  const refM = names.map((t, i) => i).filter((i) => hasLine[i]).map((i) => M[i]).sort((a, b) => b - a);
  ...
  if (marketSrc.length && refM.length) {
    const tailMean = null;
    for (let i = 0; i < nTeams; i++) {
      const vals = [];
      for (const [id] of marketSrc) {
        const rk = sourceRanks[id].ranks.get(names[i]);
        if (rk != null) vals.push(atPosition(rk, refM, tailMean));
      }
      if (vals.length) F[i] = mean(vals);
    }
  }
  // One week of lines is one line per team, which fixes a pair's DIFFERENCE and
  // nothing else (a 55-point favourite over an FCS pool reads as +27 on its own).
  // Until two weeks of lines exist the futures carry half the pillar.
  ...
  columns.push({
    ...
    label: 'Results rating: capped margins with strength of schedule, blended with a win rating',
