...
export function buildLeaderboard(rows, { population = 'all', filter = 'all' } = {}) {
  ...
  else if (filter === 'first') pool = pool.filter((r) => tryOf.get(r) === 1);
  return pool.slice()
    ...
    .slice(0, 10)
    .map((r) => ({ username: r.user_id ? r.username : guestHandleFromAnon(r.anon_id || keyOf(r)), userKey: keyOf(r), score: r.score, timeElapsed: r.time_elapsed, guessesUsed: r.guesses_used ?? null, tryNum: tryOf.get(r), playedAt: r.created_at, anon: !r.user_id }));
}
