...
import { dailyUnitOfQuizId, isArcadeQuizId, isEndGameQuizId, endGamePlan } from './daily-games.js';
...
export function buildLeaderboard(rows, { population = 'all', filter = 'all', limit = 10 } = {}) {
  ...
  const endgame = isEndGameQuizId(qid);
  const egPlan = endgame ? endGamePlan(rows) : null;
  ...
  // through to the same depth term every other board uses, because attempts
  // there would rank giving up once above fighting through five.
  const egRank = (a, b) => {
  ...
  return pool.slice()
    ...
    .slice(0, limit)
    .map((r) => ({ username: r.user_id ? r.username : guestHandleFromAnon(r.anon_id || keyOf(r)), userKey: keyOf(r), score: r.score, timeElapsed: r.time_elapsed, guessesUsed: r.guesses_used ?? null, correct: r.correct_count ?? null, priceTiebreak: r.price_tiebreak ?? null, progress: r.progress ?? null, tries: (egOf(r) || {}).tries ?? null, egTier: (egOf(r) || {}).tier ?? null, tryNum: tryOf.get(r), playedAt: r.created_at, anon: !r.user_id }));
}
