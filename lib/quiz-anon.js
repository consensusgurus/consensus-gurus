...
export function buildLeaderboard(rows, { population = 'all', filter = 'all', limit = 10 } = {}) {
  ...
  return pool.slice()
    ...
    .slice(0, limit)
    .map((r) => ({ username: r.user_id ? r.username : guestHandleFromAnon(r.anon_id || keyOf(r)), userKey: keyOf(r), score: r.score, timeElapsed: r.time_elapsed, guessesUsed: r.guesses_used ?? null, correct: r.correct_count ?? null, priceTiebreak: r.price_tiebreak ?? null, tryNum: tryOf.get(r), playedAt: r.created_at, anon: !r.user_id }));
}
