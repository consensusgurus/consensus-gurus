...
export function buildAllLeaderboard(rows) {
  ...
  return rows.slice()
    ...
    .slice(0, 10)
    .map((r) => ({ username: r.user_id ? r.username : `Player #${anonNumber(keyOf(r))}`, score: r.score, timeElapsed: r.time_elapsed, tryNum: tryOf.get(r), playedAt: r.created_at, anon: !r.user_id }));
}
