...
// Irregular plurals the suffix rules cannot reach. An explicit table, NOT a
// "-ves" rule: the stemmer is symmetric, so a blanket rule would turn "loves"
// into "lof" while "love" stays "love" and the two would never meet. Added
// 2026-09-01 after "leaf watch" missed the Leaves bucket ("leaves" stemmed to
// "leave", "leaf" stayed "leaf").
const IRREGULAR_PLURALS = new Map(Object.entries({
  leaves: 'leaf', knives: 'knife', shelves: 'shelf', wolves: 'wolf', halves: 'half',
  lives: 'life', wives: 'wife', thieves: 'thief', loaves: 'loaf', calves: 'calf',
  scarves: 'scarf', elves: 'elf', selves: 'self', hooves: 'hoof',
  feet: 'foot', teeth: 'tooth', geese: 'goose', mice: 'mouse', men: 'man',
  women: 'woman', people: 'person',
}));

// Derivational suffixes: "-ness" / "-liness" collapse to the root so a quality
// noun lands where its adjective does (cleanliness / cleanness -> clean,
// tidiness -> tidy, messiness -> messy, laziness -> lazy). Same 2026-09-01 case.
function stripNess(w) {
  if (w.length > 8 && w.endsWith('liness')) return w.slice(0, -6);
  if (w.length > 6 && w.endsWith('iness')) return w.slice(0, -5) + 'y';
  if (w.length > 6 && w.endsWith('ness')) return w.slice(0, -4);
  return w;
}
...
function stemWord(w) {
  ...
  const irr = IRREGULAR_PLURALS.get(w);
  if (irr) return irr;
  if (w.endsWith('ness')) return stripNess(w);
