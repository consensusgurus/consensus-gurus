...
function buildPdf(pageW, pageH, content) {
...
/*
  ...
  `sources` is the ordered array the page renders, each { id, short, label, tier,
  asOf, weight, ok }. Excluded sources keep their column, as on the page, so the
...
export function buildGridironPdf({ ranked, sources, tierShare, depth, fetchedAt, title, eyebrow, url }) {
  const PW = 612;      // US Letter portrait. Fits 50 rows without going landscape.
  const PH = 792;
  const M = 26;
  const c = new Content();
  ...
  // ---------- rows ----------
  const footH = 26;
  const rowH = Math.min(14, (y - M - footH) / ranked.length);
  const fs = rowH < 11 ? 6.6 : 7.6;
  ranked.forEach((r, idx) => {
    const top = y - (idx + 1) * rowH;
    if (idx % 2 === 1) c.rect(M, top, usable, rowH, C.band);
    const ty = top + rowH / 2 - fs * 0.34;

    if (r.rank <= 3) {
      c.rect(xRank - 1, top + 1, wRank - 3, rowH - 2, MEDAL[r.rank - 1]);
      c.text(xRank - 1, ty, String(r.rank), { size: fs, bold: true, color: C.white, align: 'center', width: wRank - 3 });
    } else {
      c.text(xRank - 1, ty, String(r.rank), { size: fs, color: C.slate, align: 'center', width: wRank - 3 });
    }

    c.text(xTeam, ty, fit(r.team, wTeam - 6, fs, true), { size: fs, bold: true, color: C.ink });
    c.text(xScore, ty, r.score.toFixed(1), { size: fs, bold: true, color: C.accent, align: 'right', width: wScore - 4 });
    ...
    sources.forEach((s, i) => {
      ...
      // Same deviation shading as the page, so the two tell the same story.
  ...
  // ---------- footer ----------
  const fy = y - ranked.length * rowH - 12;
  c.rect(M, fy + 8, usable, 0.6, C.rule);
  c.text(M, fy, `Consensus of every published ranking  |  built ${fetchedAt}  |  blue = source ranks the team higher than consensus, amber = lower`,
    { size: 5.8, color: C.slate });
  c.text(M, fy - 8, url, { size: 5.8, bold: true, color: C.accent });

  return buildPdf(PW, PH, c);
}
