...
export function buildGridironPdf({ ranked, sources, tierShare, depth, fetchedAt, title, eyebrow, url }) {
  ...
  const PH = 792;
  const M = 26;
  // See the note on `scoreDp` in app/GridironTable.jsx: a board rated in RUNS
  // spans about a fifth of one rated in POINTS, so a single decimal prints
  // "-0.0" for a third of its rows. Taken from the board's own range.
  const sv = ranked.map((r) => r.score);
  const scoreDp = Math.max(...sv) - Math.min(...sv) < 5 ? 2 : 1;
  ...
  const contents = pages.map((p, pi) => {
    ...
    ranked.slice(p.from, p.to).forEach((r, k) => {
      ...
      c.text(xScore, ty, (r.score > 0 ? '+' : '') + r.score.toFixed(scoreDp), { size: fs, bold: true, color: C.accent, align: 'right', width: wScore - 4 });
