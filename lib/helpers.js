...
function getSources(list, voteData, extras) {
  ...
  //
  // The flat value scales with the source's size: each unordered source has a
  // fixed 55-point budget (the total a ranked source hands out: 10+9+...+1),
  // split evenly across its n items. So a 10-item roundup gives the classic
  // 5.5/item, a 16-item roundup gives ~3.44, a 2000-item list gives a
  // near-zero 0.0275 (inclusion on a huge list means almost nothing), and a
  // short 5-item "best in class" pick gives more per item — capped at 10 so
  // no unordered mention can ever beat a ranked source's #1 (10 pts).
...
export { buildItemLink, getSources, voteKey, dedupeByName, stripItemScore };
