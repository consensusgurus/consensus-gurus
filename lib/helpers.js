...
export const SCORING_ENGINE_VERSION = '2026-06-18.1';
...
*/
function getSources(list, voteData, extras, opts) {
  const CAP = (opts && opts.limit) || 10;
  ...
  if (publications.length === 1) return publications;
  ...
  // A "decisive expert" source (`decisiveExpert: true`) sits ABOVE a true expert:
  // it counts for 1.5x the combined weight of the ordinary field, floor 6 - the
  // single dominant signal, worth more than every other source combined. Used for
  // Michelin star ratings on best-restaurant lists. The "field" excludes BOTH
  // special tiers so the two never inflate each other.
  ...
  // Raw Borda score per consensus item (additive; existing callers ignore it).
  // Used by the list detail page to derive a normalized 0-100 display score.
  const consensusScores = {};
  consensusItems.forEach((it) => { consensusScores[it] = scores[it.toLowerCase().trim()] || 0; });
