...
//
// The futures board carries ALL 138 FBS teams, not the top 50 it used to be
// truncated to. Below rank 50 it is the only market signal a team without a
// line has, and truncating it dropped 88 teams onto a shared floor where the
// board then had to invent an order for them. See CLAUDE-RANKINGS.md section 2d.
//
// ITS asOf IS 2026-08-28, NOT the day it was pulled. The ESPN mirror reachable
// on 2026-09-04 serves PRE-GAME state for games that finished a week earlier
// (it reports the Aug 29 USC game as STATUS_SCHEDULED), and all 50 prices
// already in this file came back byte-identical, so this IS the 08-28 board and
// is dated as such. Stamping it with the day it was fetched is exactly the
// "prints today's date over a stale table" failure section 3 calls actively
// dangerous. The games below are unaffected: they were verified against sources
// that are current, one game page at a time.
