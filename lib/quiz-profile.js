...
export function buildProfile(players, myKey, { signed = false, username = null } = {}) {
  ...
  return {
    ...
    tierBg: tier.bg,
    tierFg: tier.fg,
    components: { start: p.start, k: p.k, matches: p.matches, netDelta: p.netDelta, rating: p.rating, ratingRaw: p.ratingRaw != null ? p.ratingRaw : p.rating, decayDelta: p.decayDelta || 0 },
    activity: {
