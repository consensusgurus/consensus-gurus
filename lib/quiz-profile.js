// Shared builder for a single player's full IQ Points profile + activity stats.
...
export function buildProfile(players, myKey, { signed = false, username = null } = {}) {
  ...
  // that category (matches > 0). `rank` is the IQ Points rank; the rest let each
  ...
  return {
    ...
    tierBg: tier.bg,
    tierFg: tier.fg,
    // Level progress for the IQ Points panel: IQ Points into the current level vs the IQ Points the
