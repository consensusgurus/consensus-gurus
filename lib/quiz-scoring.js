...
for (const q of QUIZZES) {
  ...
  if (q && (q.format === 'timed-mcq' || q.format === 'place-map' || Number(q.maxPerQuestion) > 1)) {
    POINTS_QUIZZES.set(q.id, {
      questions: q.format === 'place-map'
        ? (Array.isArray(q.cities) ? q.cities.length : 0)
        : (Array.isArray(q.questions) ? q.questions.length : 0),
      max: q.format === 'place-map' ? (Number(q.maxPerCity) || 0) : (Number(q.maxPerQuestion) || 0),
    });
  }
}
