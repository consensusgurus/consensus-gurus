...
for (const q of QUIZZES) {
  // A quiz is points-based if it is a timed-mcq lightning round OR any quiz
  // that can award more than one point per answer (maxPerQuestion > 1). Both
  // store score as a POINTS total, not a correct-answer count.
  if (q && (q.format === 'timed-mcq' || Number(q.maxPerQuestion) > 1)) {
