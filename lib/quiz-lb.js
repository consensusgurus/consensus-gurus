...
// Rank to ADVERTISE on the "register to claim your spot" end-game CTA. It ranks
// the just-finished score against the REGISTERED leaderboard (the board a player
// actually joins by registering), but returns null unless the placement is worth
// showing: the score must be positive AND land the player ahead of at least one
// current leaderboard member (or the board is empty, i.e. they would be first).
// This stops a 0 / bottom score from being sold as a flattering "#11 on the
// leaderboard" when only a handful of players have registered.
export function registerRank(rows, score, elapsed) {
  if (score == null || score <= 0) return null;
  const list = Array.isArray(rows) ? rows : [];
  const rank = wouldBeRank(list, score, elapsed);
  if (rank == null) return null;
  return rank <= Math.max(list.length, 1) ? rank : null;
}
