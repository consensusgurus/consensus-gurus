...
import { DAILY_KEYS } from './daily-games.js';
import { QUIZZES } from './quizzes.js';
import { TROPHIES } from './trophy-defs.js';

// Daily games that are retired (kept in DAILY_KEYS so archived days still
// score) and therefore NOT required for the Completionist trophy.
const RETIRED_DAILY = new Set(['circa']);
const LIVE_DAILY_COUNT = DAILY_KEYS.filter((k) => !RETIRED_DAILY.has(k)).length;
...
export function computeTrophies(rows, players, { duelWinsByAnon = null } = {}) {
  ...
  const CRIT = {
    ...
    'daily-all': (f) => f.dailyGames.size >= LIVE_DAILY_COUNT,
