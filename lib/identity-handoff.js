...
// The PWA install path freezes a token into the home-screen entry's start_url at install
// time, and the first launch can come days later, so its tokens live much longer. Still
// bounded, and adoptable() gates the claim client-side either way. See /api/identity/pwa-token.
export const PWA_TTL_MS = 60 * 24 * 60 * 60 * 1000;
...
export async function mintHandoff(anonId, secret = process.env.IDENTITY_HANDOFF_SECRET, ttlMs = TTL_MS) {
  if (!secret || !anonId || typeof anonId !== 'string' || !/^[\w-]{1,64}$/.test(anonId)) return null;
  const payload = `${b64url(enc.encode(anonId))}.${Date.now() + ttlMs}`;
