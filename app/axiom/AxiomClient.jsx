...
//
// Scoring: 12 points. Each test over par costs 2, each wrong rule costs 3, and
...
// Exactly one candidate agrees with every tile on the board. Par is the fewest
// tests that could isolate it from the rules the gift reds leave alive.
function solveBoard(puzzle) {
  ...
  if (killers.length) {
    ...
    if (one) par = 1;
      ...
      for (let a = 0; a < testable.length && par === 3; a++) {
        for (let b = a + 1; b < testable.length; b++) {
          if (killers.every((s) => s.has(testable[a]) || s.has(testable[b]))) { par = 2; break; }
        }
      }
    }
  }
...
export default function AxiomClient({ puzzles = [], forceNum = null }) {
  ...
  const { answer: ANSWER, par: PAR } = useMemo(() => solveBoard(PUZZLE), [PUZZLE]);
    ...
    <div style={{ minHeight: '100vh', background: '#f7f8fa', position: 'relative' }}>
      ...
      <div className="ax-wrap" style={{ position: 'relative', zIndex: 2, maxWidth: 1180, margin: '0 auto', padding: '18px 38px 80px', fontFamily: SANS }}>
        <style>{`
          ...
          .ax-btn{font-family:${SANS};font-weight:800;font-size:14px;border:2px solid ${COLORS.ink};background:#fff;color:${COLORS.ink};border-radius:8px;padding:9px 16px;cursor:pointer;display:inline-flex;align-items:center;gap:7px;}
          ...
          .ax-tile{font-family:${SANS};font-weight:800;font-size:13px;letter-spacing:0.04em;border-radius:9px;padding:13px 4px;cursor:pointer;border:1.5px solid rgba(28,30,36,0.16);background:#fff;color:${COLORS.ink};text-align:center;overflow:hidden;text-overflow:ellipsis;}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap', position: 'relative', paddingRight: 28, marginBottom: 14, borderBottom: '2px solid rgba(28,30,36,0.8)', paddingBottom: 11 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 9, flexWrap: 'wrap' }}>
            <h1 style={{ margin: 0, fontFamily: MONO, fontSize: 14, letterSpacing: '0.06em', fontWeight: 500, color: COLORS.ink }}>Board No. {PUZZLE.num}</h1>
            ...
            {PUZZLE.sunday && <span style={{ fontFamily: MONO, fontSize: 9.5, letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 500, color: '#fff', background: COLORS.accent, borderRadius: 4, padding: '2px 6px' }}>Sunday Edition &middot; Wide Field</span>}
          </div>
        ...
        {preStart && (
          <div style={{ background: COLORS.cream, border: `2px solid ${COLORS.ink}`, borderRadius: 12, padding: '22px 22px', minHeight: 320, display: 'flex', flexDirection: 'column' }}>
            ...
            {gateRules ? rulesBody : (
              <div style={{ fontSize: 14, lineHeight: 1.55, color: COLORS.ink, fontWeight: 600 }}>
                <p style={{ margin: '0 0 6px' }}>{PUZZLE.tiles.length} words, one hidden rule, {PUZZLE.rules.length} candidates. Three tiles open green, two open red, and you get {PUZZLE.budget} tests to settle it. Par is {PAR}.</p>
              </div>
            )}
            <div style={{ marginTop: 'auto', paddingTop: 18 }}>
              <button className="ax-btn" onClick={startRun} style={{ background: COLORS.ink, color: '#fff', fontSize: 15, padding: '11px 22px' }}>Uncover the board</button>
            ...
            {PUZZLE.rules.map((r, i) => {
              ...
              return (
                ...
                >
                  ...
                  {winner && <span style={{ fontFamily: MONO, fontSize: 9, letterSpacing: '0.08em', textTransform: 'uppercase', fontWeight: 500, color: COLORS.green, background: '#fff', borderRadius: 4, padding: '3px 7px' }}>the rule</span>}
                </button>
              );
            })}
          </>
            ...
            <div style={{ maxWidth: 472, margin: '14px 0 12px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, background: '#fff', border: '1.5px solid rgba(28,30,36,0.18)', borderRadius: 10, padding: '12px 14px' }}>
                ...
                <span style={{ fontFamily: SANS, fontSize: 13, fontWeight: 700, color: COLORS.ink, lineHeight: 1.45 }}>
                  {g.status === 'done'
                    ? (won ? <>Named at par on {testsUsed} test{testsUsed === 1 ? '' : 's'}.</> : <>Named it on {testsUsed} test{testsUsed === 1 ? '' : 's'}{g.wrongPicks.length ? ` and ${g.wrongPicks.length} wrong name${g.wrongPicks.length === 1 ? '' : 's'}` : ''}.</>)
      ...
      {!playing && !endClosed && (
        <DailyEndCard
          ...
          subline={<>Axiom #{PUZZLE.num} &middot; {score}/{TOTAL} &middot; {testsUsed} test{testsUsed === 1 ? '' : 's'} (par {PAR}) &middot; {elapsed}</>}
          onShare={copyShare}
      ...
      {showHelp && (
        <div onClick={() => { setShowHelp(false); try { localStorage.setItem(HELP_KEY, '1'); } catch (e) {} }}
          ...
          <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 460, background: COLORS.cream, borderRadius: 12, border: `2px solid ${COLORS.ink}`, padding: '20px 22px', fontFamily: SANS, maxHeight: '86vh', overflowY: 'auto' }}>
            ...
            {rulesBody}
            <button className="ax-btn" onClick={() => { setShowHelp(false); try { localStorage.setItem(HELP_KEY, '1'); } catch (e) {} }} style={{ marginTop: 14, background: COLORS.ink, color: '#fff' }}>Play</button>
          </div>
        </div>
      )}
