  ...
  useEffect(() => {
    ...
    try {
      if (PUZZLE.num === pickPuzzle(puzzles, null).num) {
        localStorage.setItem('sot_extra_day', JSON.stringify({ d: etToday(), done: g.status !== 'playing' }));
      }
    } catch (e) {}
    ...
    <div style={{ minHeight: '100vh', background: '#f7f8fa', position: 'relative' }}>
      ...
      <div className="ex-wrap" style={{ position: 'relative', zIndex: 2, maxWidth: 1180, margin: '0 auto', padding: '18px 38px 80px', fontFamily: SANS }}>
        ...
        <div className="ex-mh" style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap', position: 'relative', paddingRight: 28, marginBottom: 16, borderBottom: '2px solid rgba(28,30,36,0.8)', paddingBottom: 11 }}>
          ...
          <div className="ex-ttl" style={{ display: 'flex', alignItems: 'baseline', gap: 9 }}>
            <h1 style={{ margin: 0, fontFamily: MONO, fontSize: 14, letterSpacing: '0.06em', fontWeight: 500, color: COLORS.ink }}>No. {PUZZLE.num}</h1>
            <span className="ex-ttl-dot" style={{ color: COLORS.faded }}>&middot;</span>
            <span className="ex-ttl-dt" style={{ fontFamily: SANS, fontStyle: 'italic', fontSize: 15, color: COLORS.faded }}>{PUZZLE.dateLabel}</span>
        ...
        {!playing && (
          <div style={{ background: '#fff', border: `2px solid ${COLORS.ink}`, borderRadius: 12, padding: '16px 16px 14px', marginBottom: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <button className="ex-btn" onClick={copyShare}><Share2 size={15} /> {copied ? 'Copied' : 'Share result'}</button>
              <button className="ex-btn" onClick={resetGame} style={{ borderColor: '#c3c8cf', color: COLORS.faded }}><RotateCcw size={15} /> Replay</button>
            </div>
            <DailyGamesPromo self="extra" refresh={g.status} />
        ...
        <div style={{ maxWidth: 620, margin: '20px auto 0' }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {[
              ...
              { n: myStats.played ? `${Math.round((myStats.perfect / myStats.played) * 100)}%` : '—', l: 'Cold Reads' },
      ...
      {justWon && (
        <div onClick={() => setJustWon(false)} style={{ position: 'fixed', inset: 0, zIndex: 85, background: 'rgba(28,30,36,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, animation: 'exfade .4s ease .3s backwards' }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: PAPER, border: `3px double ${COLORS.ink}`, borderRadius: 6, padding: '26px 30px 20px', maxWidth: 380, width: '100%', textAlign: 'center', fontFamily: SANS, boxShadow: '6px 6px 0 rgba(28,30,36,0.18)', animation: 'exstamp .45s ease .3s backwards' }}>
            <div style={{ display: 'flex', gap: 3, justifyContent: 'center', marginBottom: 12 }}>
              {'EXTRA'.split('').map((ch, i) => (
                <span key={i} style={{ width: 24, height: 24, borderRadius: 3, background: i === 1 ? COLORS.accent : COLORS.ink, color: '#fff', fontFamily: SANS, fontWeight: 900, fontSize: 15, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', boxShadow: 'inset 0 1px 3px rgba(0,0,0,0.5)' }}>{ch}</span>
              ))}
            </div>
            <div style={{ fontFamily: SANS, fontWeight: 900, fontSize: 34, color: COLORS.ink, letterSpacing: '-0.01em', margin: '2px 0 6px', lineHeight: 1.15 }}>{tears === 0 ? 'Cold read!' : 'Scoop!'}</div>
            <div style={{ fontFamily: MONO, fontSize: 12.5, color: COLORS.faded, marginBottom: 14 }}>No. {PUZZLE.num} &middot; {tears === 0 ? 'zero tears' : `${tears} tear${tears === 1 ? '' : 's'}`} &middot; {elapsed}</div>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
              <button className="ex-btn" onClick={copyShare} style={{ background: COLORS.accent, color: '#fff', borderColor: COLORS.accent }}><Share2 size={15} /> {copied ? 'Copied' : 'Share result'}</button>
              <button className="ex-btn" onClick={() => setJustWon(false)}>Read the page</button>
            </div>
            <div style={{ fontFamily: MONO, fontSize: 10, letterSpacing: '0.1em', color: COLORS.faded, marginTop: 12 }}>sourceoftruths.com/extra</div>
          </div>
        </div>
      )}
      ...
      <section style={{ display: playing ? 'none' : 'block', position: 'relative', zIndex: 2, maxWidth: 620, margin: '0 auto', padding: '10px 24px 42px', fontFamily: SANS }}>
        ...
        <p style={{ margin: 0, fontSize: 13, lineHeight: 1.65, color: COLORS.faded, fontWeight: 600 }}>
          A new front page drops every day at midnight Eastern, with a trickier story on Sundays. No app, no signup &mdash; play free in your browser, keep a streak, and race the daily leaderboard. More dailies: <a href="/circa" style={{ color: COLORS.ink, fontWeight: 800 }}>Circa</a>, our guess-the-year game, <a href="/dating" style={{ color: COLORS.ink, fontWeight: 800 }}>Dating</a>, our history-ordering game, and <a href="/crux" style={{ color: COLORS.ink, fontWeight: 800 }}>Crux</a>, our clueless crossword.
        </p>
      </section>
