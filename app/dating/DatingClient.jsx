...
export default function DatingClient({ puzzles = [], forceNum = null }) {
  ...
  const STAGE_ACC = { '--stg-acc-dk': gameColor('dating'), '--stg-acc-lt': gameColorLight('dating'), '--stg-onramp-lt': gameOnrampLight('dating') };
        ...
        {preStart && (
          <div style={{ background: STAGE ? SURF : COLORS.cream, border: STAGE ? `1px solid ${SURF_B}` : `2px solid ${COLORS.ink}`, borderRadius: 12, padding: '22px', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ marginTop: 18 }}>
              <button className="dt-btn" onClick={startGame} style={{ background: STAGE ? STAGE_C : T.cta, color: STAGE ? 'var(--stg-onramp, #08222e)' : T.white, fontSize: 15, padding: '11px 22px' }}>Start</button>
