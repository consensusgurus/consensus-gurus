...
export default function CagesClient({ puzzles = [], forceNum = null }) {
  ...
  const STAGE_ACC = { '--stg-acc-dk': gameColor('cages'), '--stg-acc-lt': gameColorLight('cages') };
    ...
    <div className={STAGE ? 'stage-page' : (LOFT ? 'loft-page' : undefined)}
      ...
      <div className="cg-wrap" style={{ position: 'relative', zIndex: 2, maxWidth: 1180, margin: '0 auto', padding: '18px 38px 80px', fontFamily: SANS }}>
        <style>{`
          ...
          .cg-tool.on{background:${COLORS.ink};color:var(--white);border-color:var(--stg-ink, ${COLORS.ink});}
        `}</style>
        ...
        {preStart && (
          <div style={{ background: STAGE ? SURF : COLORS.cream, border: STAGE ? `1px solid ${SURF_B}` : `2px solid ${COLORS.ink}`, borderRadius: 12, padding: '22px', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ marginTop: 18 }}>
              <button className="cg-btn" onClick={startGame} style={{ background: STAGE ? STAGE_C : T.cta, color: STAGE ? RAMP_INK : T.white, fontSize: 15, padding: '11px 22px' }}>Start</button>
      ...
      <section style={{ position: 'relative', display: (focusMode || STAGE) ? 'none' : 'block', zIndex: 2, maxWidth: 620, margin: '0 auto', padding: '10px 24px 42px', fontFamily: SANS }}>
        ...
        <p style={{ margin: 0, fontSize: 13, lineHeight: 1.65, color: FADED, fontWeight: 600 }}>
          A new puzzle drops every day at midnight Eastern, and Sundays step up to a harder Edition with fewer, bigger cages. No app, no signup &mdash; play free in your browser, keep a streak, and race the daily leaderboard. The other two sudokus: <a href="/cages" style={{ color: INK, fontWeight: 800 }}>Cages</a>, the classic 9×9, and <a href="/quilt" style={{ color: INK, fontWeight: 800 }}>Quilt</a>, the jigsaw one with the boxes redrawn.
        </p>
      </section>
