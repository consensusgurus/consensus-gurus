...
// (how many of the 6,000 words are closer). Keep guessing, unlimited, until you
// land the exact word. Fewer guesses is better; the daily leaderboard ranks on
// fewest guesses, ties broken by fastest time.
//
// Proximity is precomputed (word2vec) per day into `active.order` — the 6,000
// VOCAB indices sorted most-similar-first — so the browser needs no model. The
// answer itself is never sent as a string (it's VOCAB[order[0]], only for the
// live day). Same daily plumbing as every other game: Eastern-date gating on
// the server, per-puzzle localStorage saves, /warmer?p=N archive, streaks +
// stats, and the shared /api/quiz/* board flow. One free hint (a warm word) is
// offered to UNREGISTERED players only, so it never assists a leaderboard entry.

import React, { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import { useSearchParams } from 'next/navigation';
import { HelpCircle, Share2, X, Lightbulb, Smartphone, Thermometer, CornerDownLeft } from 'lucide-react';
...
const N_WORDS = VOCAB.length; // 6000

// Cold → hot temperature bands, keyed by proximity rank (1 = the answer).
// Colors run blue → cyan → lime → amber → orange → red so the spectrum reads
// left-cold, right-hot. `emoji` drives the shareable heat trail.
const BANDS = [
  { name: 'Found it',  min: 1,    max: 1,    color: '#15803d', emoji: '✅' },
  { name: 'Scorching', min: 2,    max: 10,   color: '#dc2626', emoji: '🟥' },
  { name: 'Hot',       min: 11,   max: 50,   color: '#ea580c', emoji: '🟧' },
  { name: 'Warm',      min: 51,   max: 200,  color: '#f59e0b', emoji: '🟨' },
  { name: 'Tepid',     min: 201,  max: 700,  color: '#84cc16', emoji: '🟩' },
  { name: 'Cool',      min: 701,  max: 2000, color: '#0ea5e9', emoji: '🟦' },
  { name: 'Cold',      min: 2001, max: 1e9,  color: '#3b5bdb', emoji: '⬜' },
];
function bandFor(rank) {
  for (const b of BANDS) if (rank >= b.min && rank <= b.max) return b;
  return BANDS[BANDS.length - 1];
}
// Bar fill fraction on a log scale — rank 1 fills it, the coldest word barely does.
function heatFrac(rank) {
  const f = 1 - Math.log(rank) / Math.log(N_WORDS);
...
// Score = a 0..100 completion that rewards efficiency: 100 for a 1-guess solve,
// decaying ~1.5%/guess to a floor of 20 (every solver keeps a real completion).
// Submitted with total=100 and the true guess count so the daily board's
// (score desc, then guesses, then time) tiebreak resolves to fewest guesses,
// fastest time. See lib/daily-combined.js.
function scoreForGuesses(g) {
  return Math.max(20, Math.min(100, Math.round(100 * Math.pow(0.985, Math.max(0, g - 1)))));
}
...
function deriveStats(s, todayNum) {
  ...
  const gcounts = nums.map((n) => rec[n].g).filter((x) => Number.isFinite(x));
  const best = gcounts.length ? Math.min(...gcounts) : null;
  const avg = gcounts.length ? Math.round(gcounts.reduce((a, b) => a + b, 0) / gcounts.length) : null;
  let max = 0, run = 0, prev = null;
  for (const n of nums) { run = prev != null && n === prev + 1 ? run + 1 : 1; if (run > max) max = run; prev = n; }
  let cur = 0, at = rec[todayNum] ? todayNum : todayNum - 1;
  while (rec[at]) { cur++; at--; }
  return { played, best, avg, cur, max };
}
function mergeServerStats(s, recent, puzzles) {
  ...
  for (const m of recent) {
    ...
    rec[p.num] = { g: null, t: null, won: true }; // solved on the server; guess count unknown here
  }
  if (!changed) return s;
  const s2 = { ...s, rec };
  try { localStorage.setItem(STATS_KEY, JSON.stringify(s2)); } catch (e) {}
...
function freshState() {
  return { v: 1, guesses: [], status: 'playing', hintUsed: false, t0: null, tEnd: null };
}
...
export default function WarmerClient({ active, puzzles = [], forceNum = null }) {
  ...
  // rankByVocabIndex[vocabIndex] = 0-based position in the similarity order, so a
  // guess resolves to a rank in O(1) instead of scanning the 6,000-long list.
  ...
  const [lastRank, setLastRank] = useState(null);   // rank of the most recent guess (for the pinned highlight)
  ...
  const todayNum = useMemo(() => {
    const t = etToday();
    const open = puzzles.filter((p) => p.live <= t);
    return open.length ? open[open.length - 1].num : (puzzles[0] ? puzzles[0].num : PUZZLE.num);
  }, [puzzles, PUZZLE.num]);
    ...
    try {
      ...
      if (raw) {
        ...
        if (saved && saved.v === 1 && Array.isArray(saved.guesses)) {
          ...
          if (saved.guesses.length) setLastRank(saved.guesses[saved.guesses.length - 1].rank);
        }
      }
  ...
  useEffect(() => {
    ...
    try {
      if (isTodays) localStorage.setItem('sot_warmer_day', JSON.stringify({ d: etToday(), done: g.status !== 'playing' }));
    } catch (e) {}
  ...
  // ---- identity + leaderboard (same /api/quiz/* flow as every other board) ----
  useEffect(() => {
  ...
  const rankOf = useCallback((word) => {
    const vi = VOCAB_INDEX[word];
    if (vi == null) return null;
    const pos = rankByVocab[vi];
  ...
  function postResult(g2) {
    ...
    const score = scoreForGuesses(gc);
    try { setStats(recordStat(PUZZLE.num, { g: gc, t: el, won: true })); } catch (e) {}
    try {
      fetch('/api/quiz/result', {
        method: 'POST', keepalive: true, headers: { 'Content-Type': 'application/json' },
        // guessesUsed = total guesses, so the daily leaderboard (score, then
        // guesses, then time) ranks on fewest guesses, ties by fastest time.
        body: JSON.stringify({ quizId: PUZZLE.quizId, score, total: 100, correct: 1, guessesUsed: gc, timeElapsed: el, email: identity?.email || undefined, anonId: getAnonId(), isMobile: isMobileDevice(), referrer: (typeof document !== 'undefined' ? document.referrer : '') }),
      })
        .then((r) => r.json())
        .then((d) => { if (d && !d.error) setBoard({ ...EMPTY_BOARD, ...d }); })
        .catch(() => {});
  ...
  function commitGuess(wordRaw) {
    ...
    if (existing) { setLastRank(existing.rank); say(`Already guessed — ${word} is #${existing.rank}.`); return; }
  ...
  // one free hint (UNREGISTERED players only): surface a genuinely warm word
  // (around rank 25) the player hasn't tried, and drop it into their guesses.
  function useHint() {
    if (!playing || g.hintUsed) return;
    const tried = new Set(guesses.map((x) => x.w));
    let pick = null;
    for (const targetPos of [24, 20, 30, 15, 40, 12, 50]) {
      const w = VOCAB[ORDER[targetPos]];
      if (w && !tried.has(w)) { pick = { w, rank: targetPos + 1 }; break; }
    }
  ...
  function resetGame() {
    ...
    setG(freshState()); setLastRank(null); setText(''); setJustWon(false); setEndClosed(false);
    setTimeout(() => { try { inputRef.current && inputRef.current.focus(); } catch (e) {} }, 30);
  }

  // guesses sorted hottest-first for the running list
  ...
  function shareText() {
    ...
    // heat trail: downsample the guess path to ~10 squares, ending in the win
    ...
    <div style={{ minHeight: '100vh', background: COLORS.cream, position: 'relative' }}>
      ...
      <div className="wm-wrap" style={{ position: 'relative', zIndex: 2, maxWidth: 1180, margin: '0 auto', padding: '18px 38px 80px', fontFamily: SANS }}>
        <style>{`
          ...
          .wm-meta{display:flex;align-items:center;gap:14px;font-family:${MONO};font-size:11.5px;letter-spacing:.08em;text-transform:uppercase;color:${COLORS.faded};margin:2px 0 14px;flex-wrap:wrap;}
        ...
        <div style={{ maxWidth: 620, margin: '0 auto' }}>
          <div style={{ display: playing ? 'block' : 'none' }}><DailyTopNav player={player} compact /></div>
          {!playing && <DailyTopNav player={player} />}
            ...
            {playing ? (
              <>
                ...
                <div className="wm-meta">
                  ...
                  {!identity && !g.hintUsed && (
                    <button onClick={useHint} title="Reveal one warm word (one hint per puzzle)" style={{ marginLeft: 'auto', background: COLORS.accentSoft, border: '1.5px solid rgba(220,38,38,0.45)', color: '#9a1c1c', fontFamily: SANS, fontWeight: 800, fontSize: 12, borderRadius: 8, padding: '6px 11px', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
              ...
              <div style={{ textAlign: 'center', padding: '6px 0 4px' }}>
                ...
                <div style={{ fontFamily: SANS, fontWeight: 700, fontSize: 13.5, color: COLORS.faded }}>{won ? <>Solved in {guesses.length} guess{guesses.length === 1 ? '' : 'es'} &middot; {elapsed}{g.hintUsed ? ' · 1 hint' : ''}</> : null}</div>
              </div>
            )}
          </div>
          ...
          <div id="daily-leaderboard" style={{ maxWidth: 620, margin: '26px auto 0', background: '#fff', border: '1.5px solid rgba(20,22,28,0.12)', borderRadius: 12, padding: '14px 16px' }}>
        ...
        <div onClick={() => { setShowHelp(false); try { localStorage.setItem(HELP_KEY, '1'); } catch (e) {} }} style={{ position: 'fixed', inset: 0, background: 'rgba(20,22,28,0.55)', zIndex: 70, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 460, background: COLORS.cream, borderRadius: 12, border: `2px solid ${COLORS.ink}`, padding: '20px 22px', fontFamily: SANS, maxHeight: '86vh', overflowY: 'auto' }}>
            ...
            <div style={{ fontSize: 14, lineHeight: 1.55, color: COLORS.ink, fontWeight: 600 }}>
              ...
              <p style={{ margin: '0 0 9px' }}>Guesses are <b>unlimited</b>, so you always finish. The daily leaderboard ranks players by <b>fewest guesses</b>, with fastest time breaking ties. One free <b>hint</b> reveals a warm word.</p>
      ...
      <section style={{ position: 'relative', zIndex: 2, maxWidth: 620, margin: '0 auto', padding: '10px 24px 42px', fontFamily: SANS }}>
        ...
        <p style={{ margin: '0 0 8px', fontSize: 13, lineHeight: 1.65, color: COLORS.faded, fontWeight: 600 }}>
          Closeness is semantic, not alphabetical: for the answer &ldquo;sea,&rdquo; words like ocean, waves, and coast run hot, while unrelated words stay cold. Guesses are unlimited, so everyone reaches the answer &mdash; the challenge is getting there in as few guesses as you can. The daily leaderboard ranks players on fewest guesses, with the fastest time breaking ties.
        </p>
