...
function Home({ lists, viewCounts, voteData, extras, trending = {}, openList, onSubmit }) {
  ...
  const shuffledQuizzes = useMemo(() => {
    ...
    const eligible = FACTUAL_QUIZZES.filter((q) => q.id === 'nyc-restaurant-geo-guesser' || q.id === 'nyc-restaurant-geo-guesser-pt-2' || q.id === 'nyc-restaurant-geo-guesser-pt-3' || q.id === 'nyc-pizza-geo-finder' || q.id === 'nyc-burger-geo-finder' || q.id === 'nyc-parks-geo-finder' || q.id === 'boston-restaurant-geo-guesser' || (quizPlays[q.id] || 0) >= QUIZ_TILE_MIN_PLAYS);
