...
// tally-*, suds-*, circa-*, extra-*, carve-*, stet-*, outwit-*, tuck-*, alibi-*, cipher-*, closer-*) and every entry in a family shares ONE hero image, so the two hero
// tiles (Newest + Trending) must never both draw from the same family.
const DAILY_GAME_FAMILY_RE = /^(crux|emcee|garble|links|span|dating|tally|suds|circa|extra|carve|stet|outwit|tuck|alibi|cipher|closer)-/;
function gameFamily(id) { const m = (id || '').match(DAILY_GAME_FAMILY_RE); return m ? m[1] : null; }
// Rule: daily games (Crux, Emcee, Garble, Links, Span, Dating, Tally, Suds, Circa, Extra, Carve, Stet, Outwit, Tuck, Alibi, Cipher, Closer) publish a fresh
