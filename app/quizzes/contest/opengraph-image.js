...
export default async function Image() {
  ...
  const place = (label, amount, lead) => h('div', {
    ...
    style: {
      ...
      borderRadius: 14,
      background: lead ? '#fdf2ef' : '#fbfaf9',
      border: lead ? '2px solid #f2cfc4' : '2px solid #eef0f4',
      marginRight: 12,
    },
  }, [
    T(label, { fontSize: 19, fontWeight: 700, color: '#716d79', marginBottom: 2 }),
    T(amount, { fontSize: lead ? 44 : 32, fontWeight: 800, color: lead ? '#3a4152' : '#14141a', letterSpacing: '-1px' }),
  ])

  const card = h('div', {
    style: {
      width: '100%', height: '100%', display: 'flex', flexDirection: 'column',
      justifyContent: 'space-between', background: '#fbfaf9', fontFamily: 'Manrope',
    ...
    h('div', { key: 'top', style: { display: 'flex', flexDirection: 'column' } }, [
      h('div', { key: 'row', style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between' } }, [
        ...
        T(`${left} DAYS LEFT`, { fontSize: 22, fontWeight: 700, letterSpacing: 3, color: '#3a4152' }),
      ]),
    ...
    h('div', { key: 'mid', style: { display: 'flex', flexDirection: 'column' } }, [
      T(`Win ${CONTEST.prizeLabel}.`, { fontSize: 92, fontWeight: 800, letterSpacing: '-2px', color: '#14141a', lineHeight: 1.0, marginBottom: 10 }),
      T('Bring the most new players to Mind Loft.', { fontSize: 30, fontWeight: 600, color: '#716d79', lineHeight: 1.25, marginBottom: 20 }),
    ...
    h('div', { key: 'ft', style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid #e4e0dd', paddingTop: '16px', fontSize: 19 } }, [
      T(`Free to enter. Ends ${CONTEST.deadlineLabel}.`, { color: '#716d79', fontWeight: 600 }),
      T('MINDLOFTDAILY.COM', { color: '#3a4152', fontWeight: 700 }),
    ]),
  ])
