...
export default async function Image() {
  ...
  const place = (label, amount, lead) => h('div', {
    ...
    T(amount, { fontSize: lead ? 44 : 32, fontWeight: 800, color: lead ? '#1e3a8a' : '#0b0c0e', letterSpacing: '-1px' }),
  ])
  ...
  const card = h('div', {
    ...
    h('div', { key: 'mid', style: { display: 'flex', flexDirection: 'column' } }, [
      T(`Win ${CONTEST.prizeLabel}.`, { fontSize: 92, fontWeight: 800, letterSpacing: '-2px', color: '#0b0c0e', lineHeight: 1.0, marginBottom: 10 }),
