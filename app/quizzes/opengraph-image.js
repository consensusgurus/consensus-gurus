...
function iconRingsDataURI() {
  const mark = '<g transform="translate(56,59) scale(1.74)">' + "<path d=\"M20 52l40-34 40 34\" stroke=\"#14141a\" stroke-width=\"7\" stroke-linecap=\"round\" stroke-linejoin=\"round\" fill=\"none\"/><path d=\"M14 102h92\" stroke=\"#14141a\" stroke-width=\"6\" stroke-linecap=\"round\"/><g transform=\"translate(31,48) scale(0.53)\"><path d=\"M14 42C12 28 22 16 36 16C40 8 54 6 60 14C70 8 84 14 86 26C96 30 98 44 88 50C92 58 86 66 76 64C74 72 64 74 60 66C48 70 36 66 32 56C20 56 12 50 14 42Z\" fill=\"#c04a34\"/></g>" + '</g>'
...
export default async function Image() {
  ...
  const card = h('div', { style: { width: '100%', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', background: '#fbfaf9', fontFamily: 'Manrope', padding: '48px 72px', position: 'relative' } }, [
    ...
    h('div', { key: 'top', style: { display: 'flex', flexDirection: 'column' } }, [
      h('div', { key: 'hd', style: { display: 'flex', flexDirection: 'column', marginBottom: 20 } }, [
        h('div', { key: 'row', style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between' } }, [
          ...
          T('The Quizzes', { fontSize: 22, fontWeight: 700, letterSpacing: 3, color: '#3a4152', textTransform: 'uppercase' }),
        ]),
        h('div', { key: 'l1', style: { display: 'flex', width: '100%', height: '2px', background: '#e4e0dd', marginTop: '10px' } }),
        h('div', { key: 'l2', style: { display: 'flex', width: '210px', height: '4px', background: '#c04a34', marginTop: '3px' } }),
      ]),
      T('Elevate Your Thinking.', { fontSize: 74, fontWeight: 800, letterSpacing: '-1.5px', color: '#14141a', lineHeight: 1.0, marginBottom: 14 }),
      T(`${count} timed quizzes across film, music, sports, and beyond. Name them, match them, map them, beat the clock.`, { fontSize: 26, fontWeight: 600, color: '#716d79', lineHeight: 1.3, maxWidth: '92%' }),
    ]),
    h('div', { key: 'feat', style: { display: 'flex', flexDirection: 'column' } },
      SAMPLES.map((s, i) => h('div', { key: i, style: { display: 'flex', alignItems: 'center', marginBottom: '8px' } }, [
        h('div', { key: 't', style: { display: 'flex', width: 36, alignItems: 'center' } }, [
          h('div', { key: 'tri', style: { width: 0, height: 0, borderTop: '9px solid transparent', borderBottom: '9px solid transparent', borderLeft: '15px solid #3a4152' } }),
        ]),
        h('div', { key: 'n', style: { display: 'flex', fontSize: 30, fontWeight: 600, color: '#14141a', lineHeight: 1.15 } }, s),
      ]))
    ),
    h('div', { key: 'ft', style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid #e4e0dd', paddingTop: '16px', fontSize: 19 } }, [
      T('Beat the clock, then the leaderboard.', { color: '#716d79', fontWeight: 600 }),
      T('PLAY AT MINDLOFTDAILY.COM/QUIZZES', { color: '#3a4152', fontWeight: 700 }),
    ]),
  ])
