...
// of the account's devices. Qualifying window (both sides): plays after the
// duel was created, plus a GRACE_MS window before creation, so a round finished
// just before creating or accepting the challenge still counts, while an old
// score can never settle a fresh duel.
export async function POST(request) {
  try {
    ...
    // Both sides get a 60-min pre-creation grace window: the challenger usually
