...
// POST /api/duel/cancel  { token, anonId, email? }
// The CHALLENGER (any browser of their account) dismisses their own duel.
export async function POST(request) {
  try {
    ...
    if (!new Set(anons).has(duel.challenger_anon)) return NextResponse.json({ error: 'only_challenger_can_cancel' }, { status: 403 });
