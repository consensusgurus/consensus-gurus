...
// GET /api/quiz/totals -> { total, byQuiz: {id:count}, recent7: {id:count} }
...
export async function GET() {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ total: 0, byQuiz: {}, recent7: {} });
    }
    const rows = data || [];
    const byQuiz = {};
    const recent7 = {};
    ...
    for (const r of rows) {
      ...
      if (r.created_at && new Date(r.created_at).getTime() >= cutoff) {
        recent7[r.quiz_id] = (recent7[r.quiz_id] || 0) + 1;
      }
    }
    return NextResponse.json({ total: rows.length, byQuiz, recent7 });
  } catch (e) {
    return NextResponse.json({ total: 0, byQuiz: {}, recent7: {} });
  }
}
