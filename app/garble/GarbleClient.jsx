...
export default function GarbleClient({ puzzles = [], forceNum = null }) {
  ...
  const STAGE_ACC = { '--stg-acc-dk': gameColor('garble'), '--stg-acc-lt': gameColorLight('garble'), '--stg-onramp-lt': gameOnrampLight('garble') };
    ...
    <div className={STAGE ? 'stage-page' : (LOFT ? 'loft-page' : undefined)}
      ...
      <div className="gb-wrap" style={{ position: 'relative', zIndex: 2, maxWidth: 1180, margin: '0 auto', padding: '18px 38px 80px', fontFamily: SANS }}>
        ...
        <div className={LOFT && !STAGE ? 'loft-stage' : undefined}>
          ...
          {preStart && (
            <div className={STAGE ? 'stg-board' : (LOFT ? 'loft-card' : undefined)} style={{ background: STAGE ? SURF : COLORS.cream, border: STAGE ? `1px solid ${SURF_B}` : `2px solid ${COLORS.ink}`, borderRadius: 12, padding: '22px', display: 'flex', flexDirection: 'column', marginBottom: 16 }}>
              ...
              <div style={{ marginTop: 18 }}>
                <button className="gb-btn" onClick={startGame} style={{ background: STAGE ? STAGE_C : T.cta, color: STAGE ? 'var(--stg-onramp, #08222e)' : T.white, fontSize: 15, padding: '11px 22px' }}>Start</button>
