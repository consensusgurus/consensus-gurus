...
// Unpark launched 2026-07-30 as the 34th daily (as "Park"; renamed 2026-07-31,
// with /park 308ing here). It is linked from the hub puzzles row, the footer,
// the /daily archive, and the sitemap (/unpark is the canonical, evergreen URL).
...
export const metadata = {
  title: 'Unpark — Free Daily Sliding Block Puzzle | Source of Truths',
  description:
  ...
  alternates: { canonical: '/unpark' },
  openGraph: {
    title: 'Unpark — A Daily Sliding Block Puzzle',
    ...
    url: '/unpark', type: 'website', siteName: 'Source of Truths',
  },
  twitter: {
...
const gameJsonLd = {
  '@context': 'https://schema.org', '@type': 'Game', name: 'Unpark',
  alternateName: 'Unpark — Daily Sliding Block Puzzle',
  url: 'https://sourceoftruths.com/unpark',
  description:
...
const breadcrumbJsonLd = {
  ...
  itemListElement: [
    ...
    { '@type': 'ListItem', position: 3, name: 'Unpark' },
  ],
};
    ...
    <div style={{ minHeight: '100vh', background: '#f7f8fa', fontFamily: "'Manrope', system-ui, sans-serif", display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ textAlign: 'center', maxWidth: 420 }}>
        <div style={{ display: 'flex', gap: 5, justifyContent: 'center', marginBottom: 18 }}>
          {'UNPARK'.split('').map((ch, i) => (
            <div key={i} style={{ width: 40, height: 40, borderRadius: 5, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontSize: 23, background: i < 2 ? '#7c5c2e' : '#1c1e24', color: '#fff', boxShadow: 'inset 0 2px 5px rgba(0,0,0,0.5), 0 1px 0 rgba(255,255,255,0.65)' }}>{ch}</div>
          ))}
        </div>
        <h1 style={{ fontSize: 24, fontWeight: 800, color: '#1c1e24', margin: '0 0 8px' }}>Unpark launches {first ? first.dateLabel : 'soon'}.</h1>
...
export default function UnparkPage({ searchParams }) {
  ...
  return (
    <>
      ...
      <Suspense fallback={null}>
        <UnparkClient key={forceNum || 'today'} puzzles={visiblePuzzles} forceNum={forceNum} />
      </Suspense>
    </>
  );
}
