...
export default function StreakClient({ puzzles = [], questionsByNum = {}, forceNum = null }) {
  ...
  const STAGE_ACC = { '--stg-acc-dk': gameColor('streak'), '--stg-acc-lt': gameColorLight('streak'), '--stg-onramp-lt': gameOnrampLight('streak') };
        ...
        {preStart && (
          <div className={STAGE ? 'stg-gate' : undefined} style={{ background: STAGE ? SURF : COLORS.cream, border: STAGE ? `1px solid ${SURF_B}` : `2px solid ${COLORS.ink}`, borderRadius: 12, padding: '22px', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ marginTop: 18, display: 'flex', flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
              <button className="sk-btn" onClick={startGame} style={{ background: STAGE ? STAGE_C : T.cta, color: STAGE ? 'var(--stg-onramp, #08222e)' : T.white, fontSize: 15, padding: '11px 22px' }}>Start</button>
              <div>
          ...
          {!playing && (
            <div style={{ maxWidth: 472, margin: '0 auto' }}>
              <div style={{ fontSize: 15, fontWeight: 800, color: INK, margin: '8px 0 0' }}>
                {won ? <>A perfect run: <span style={{ color: ACC }}>40 straight</span>.</> : <>You cleared <span style={{ color: ACC }}>{depth} of {TOTAL_Q}</span>.</>}
              </div>
