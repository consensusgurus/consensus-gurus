    ...
    <div className={LOFT ? 'loft-page' : undefined} style={{ minHeight: '100vh', background: T.surface, position: 'relative', overflowX: LOFT ? 'hidden' : undefined }}>
      ...
      <div className="sho-wrap" style={{ position: 'relative', zIndex: 2, maxWidth: 1180, margin: '0 auto', padding: '18px 38px 80px', fontFamily: SANS }}>
        <style>{`
          ...
          .sho-btn{font-family:${SANS};font-weight:800;font-size:14px;border:2px solid var(--blue-deep);background:var(--white);color:var(--blue-deep);border-radius:8px;padding:9px 16px;cursor:pointer;display:inline-flex;align-items:center;gap:7px;}
          ...
          .sh-tot.made{background:rgba(255,255,255,0.92);color:${COLORS.ink};border-color:rgba(255,255,255,0.9);font-weight:700;}
          ...
          .sh-act{font-family:${SANS};font-weight:800;font-size:15px;letter-spacing:0.02em;border:2px solid rgba(255,255,255,0.9);background:rgba(255,255,255,0.94);color:${COLORS.ink};border-radius:9px;padding:12px 0;flex:1 1 0;cursor:pointer;}
          .sh-act:active{transform:translateY(1px);}
          .sh-act:disabled{opacity:0.35;cursor:default;}
          .sh-act.gold{background:#f4d98d;border-color:#e8b43a;color:#5b4104;}
        ...
        {preStart && (
          <div style={{ background: COLORS.cream, border: `2px solid ${COLORS.ink}`, borderRadius: 12, padding: '22px', display: 'flex', flexDirection: 'column' }}>
            <div style={{ fontSize: 20, fontWeight: 800, color: COLORS.ink, marginBottom: 10 }}>{gateRules ? 'How to play' : 'Shoe is ready'}</div>
            {gateRules ? rulesBody : (
            ...
            <div style={{ marginTop: 18 }}>
              <button className="sho-btn" onClick={startGame} style={{ background: T.cta, color: T.white, fontSize: 15, padding: '11px 22px' }}>Deal the first hand</button>
        ...
        <div className={LOFT ? 'loft-card' : undefined} style={{ background: T.white, border: `2px solid ${COLORS.ink}`, borderRadius: 10, padding: '13px 15px 15px', boxShadow: '5px 5px 0 rgba(28,30,36,0.16)', marginBottom: 12 }}>
          ...
          {R.hands.length > (playing && cur && !cur.settled ? 1 : 0) && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10, flexWrap: 'wrap' }}>
              {R.hands.map((hd, i) => (!hd.settled ? null : (
                <span key={i} className={`sh-pill ${hd.net > 0 ? (hd.net === 15 ? 'bj' : 'w') : hd.net < 0 ? 'l' : ''}`} style={{ color: COLORS.ink, background: hd.net > 0 ? (hd.net === 15 ? '#fdf3d8' : '#e7f6ec') : hd.net < 0 ? '#fbeaea' : '#f1f3f6', borderColor: 'rgba(28,30,36,0.2)' }}>
                  #{i + 1} {fmtChips(hd.net)}
                </span>
              )))}
              <span style={{ marginLeft: 'auto', fontFamily: MONO, fontSize: 11.5, color: COLORS.faded }}>bank <b style={{ color: chips >= 0 ? COLORS.green : COLORS.rust, fontWeight: 700 }}>{fmtChips(chips)}</b></span>
            </div>
          )}
          ...
          {started && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 11, flexWrap: 'wrap', fontFamily: SANS, fontSize: 12, fontWeight: 700, color: COLORS.faded }}>
          ...
          {!playing && (
            <div style={{ margin: '0 auto' }}>
              <div style={{ fontSize: 13.5, fontWeight: 700, color: COLORS.ink, margin: '12px 0 0' }}>
                Bank {fmtChips(chips)} against par {fmtChips(PUZZLE.par)}{won ? ': par beaten.' : ': under par.'} {R.busts > 0 ? `${R.busts} bust${R.busts === 1 ? '' : 's'}.` : 'No busts.'}
              </div>
              <div style={{ fontSize: 12.5, fontWeight: 600, color: COLORS.faded, fontStyle: 'italic', margin: '8px 0 0' }}>
              ...
              {PUZZLE.sunday && (
                <div style={{ fontSize: 12.5, fontWeight: 600, color: COLORS.faded, fontStyle: 'italic', margin: '8px 0 0' }}>The Sunday Edition: seven hands off the entire 52-card deck.</div>
              )}
              ...
              <p className="loft-tailnote" style={{ fontSize: 12, color: COLORS.faded, fontWeight: 600, margin: '12px 0 0' }}>
                {isTodays ? (
                  <>
                    {countdown ? <>Next Shoe in <b style={{ color: COLORS.ink, fontVariantNumeric: 'tabular-nums' }}>{countdown}</b>.</> : 'A new shoe is dealt at midnight Eastern.'}
                    {prevPuzzle && (
                      <>
                        {' '}Meanwhile:{' '}
        ...
        <GamePanel self="shoe" name="Shoe" onShow={() => setShowChrome(true)} />
        ...
        {showA2hsHelp && (
          <div onClick={() => setShowA2hsHelp(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(20,22,28,0.55)', zIndex: 90, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 18 }}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: T.white, borderRadius: 14, maxWidth: 430, width: '100%', padding: '22px 22px 16px', fontFamily: SANS, border: '1.5px solid rgba(20,22,28,0.12)' }}>
              <div style={{ fontSize: 17, fontWeight: 800, color: COLORS.ink, marginBottom: 8 }}>Add Shoe to your Home Screen</div>
              {isIosDevice() ? (
      ...
      <section style={{ position: 'relative', display: focusMode ? 'none' : 'block', zIndex: 2, maxWidth: 620, margin: '0 auto', padding: '10px 24px 42px', fontFamily: SANS }}>
        <h2 style={{ margin: '0 0 8px', fontSize: 15, fontWeight: 800, letterSpacing: '-0.01em', color: COLORS.ink }}>About Shoe</h2>
        ...
        <p style={{ margin: 0, fontSize: 13, lineHeight: 1.65, color: COLORS.faded, fontWeight: 600 }}>
          A new shoe is dealt every day at midnight Eastern, and Sundays step up to seven hands off the entire 52-card deck, where a perfect counter knows exactly what remains. No app, no signup &mdash; play free in your browser, keep a streak, and race the leaderboard. For more cards, try <a href="/taire" style={{ color: COLORS.ink, fontWeight: 800 }}>Taire</a>, the daily solitaire, or <a href="/hands" style={{ color: COLORS.ink, fontWeight: 800 }}>Hands</a>, the daily poker solitaire.
        </p>
      </section>
