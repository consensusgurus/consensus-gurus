...
export default function QuiltClient({ puzzles = [], forceNum = null }) {
  ...
  const STAGE_ACC = { '--stg-acc-dk': gameColor('quilt'), '--stg-acc-lt': gameColorLight('quilt') };
    ...
    <div className={STAGE ? 'stage-page' : (LOFT ? 'loft-page' : undefined)}
      ...
      <div className="ql-wrap" style={{ position: 'relative', zIndex: 2, maxWidth: 1180, margin: '0 auto', padding: '18px 38px 80px', fontFamily: SANS }}>
        <style>{`
          ...
          .ql-pad{width:100%;aspect-ratio:1;border-radius:9px;border:1.5px solid rgba(28,30,36,0.5);background:var(--white);font-family:${MONO};font-weight:500;color:${INK};cursor:pointer;display:flex;align-items:center;justify-content:center;position:relative;box-shadow:0 2px 0 rgba(28,30,36,0.4);}
        ...
        {preStart && (
          <div style={{ background: STAGE ? SURF : COLORS.cream, border: STAGE ? `1px solid ${SURF_B}` : `2px solid ${COLORS.ink}`, borderRadius: 12, padding: '22px', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ marginTop: 18 }}>
              <button className="ql-btn" onClick={startGame} style={{ background: STAGE ? STAGE_C : T.cta, color: STAGE ? RAMP_INK : T.white, fontSize: 15, padding: '11px 22px' }}>Start</button>
