...
// Source chip palette by a hash of the label, so each publication is stable.
...
function shortLabel(label) {
  return String(label || '')
    .replace(/\s*[·|–—-].*$/, '')
    .replace(/\s*\(.*$/, '')
    .replace(/\s*(Guide|Reviews?|Ranked by Rating).*$/i, '')
    .trim();
}
...
// Source chips for an item: which publications rank it (deduped short labels).
...
export default function RankingView({ list, voteData, extras }) {
  ...
  // Non-consensus modes: simple ranked list, no Borda score / chips.
      ...
      <div style={{ fontFamily: FONT }}>
        <div style={{ background: '#fff', border: `1px solid ${C.line}`, borderRadius: 12, overflow: 'hidden' }}>
          {items.map((item, i) => {
            ...
            return (
              <div key={item} style={{ display: 'flex', gap: 13, alignItems: 'flex-start', padding: '12px 14px', borderTop: i === 0 ? 'none' : `1px solid ${C.line}` }}>
                <span style={{ flex: 'none', width: 24, fontWeight: 800, fontSize: 16, color: C.ink, textAlign: 'center' }}>{i + 1}</span>
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 6 }}>
            {podium.map((item, i) => {
              ...
              const big = i === 0;
              return (
                <div key={item} style={{ gridColumn: big ? '1 / -1' : 'auto', position: 'relative', borderRadius: 14, overflow: 'hidden', minHeight: big ? 230 : 200, display: 'flex', backgroundImage: src ? `url("${src}")` : grad(name), backgroundSize: 'cover', backgroundPosition: 'center' }}>
                  ...
                  <div style={{ position: 'relative', alignSelf: 'flex-end', padding: '16px 18px', color: '#fff', width: '100%' }}>
                    <div style={{ fontSize: big ? 24 : 19, fontWeight: 800, letterSpacing: '-0.01em', lineHeight: 1.15 }}>{name}</div>
                    {locality && <div style={{ fontSize: 12, opacity: 0.85, margin: '3px 0 11px' }}>{locality}</div>}
        ...
        <>
          ...
          <div style={{ background: '#fff', border: `1px solid ${C.line}`, borderRadius: 12, overflow: 'hidden' }}>
            {rest.map((item, idx) => {
              ...
              const src = heroUrl(heroMap, item);
              return (
                <div key={item} style={{ display: 'flex', gap: 14, alignItems: 'flex-start', padding: '12px 14px', borderTop: idx === 0 ? 'none' : `1px solid ${C.line}` }}>
