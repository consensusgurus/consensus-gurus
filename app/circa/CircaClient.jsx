...
export default function CircaClient({ puzzles = [], forceNum = null }) {
  ...
  useEffect(() => {
    ...
    try {
      if (PUZZLE.num === pickPuzzle(puzzles, null).num) {
        localStorage.setItem('sot_circa_day', JSON.stringify({ d: etToday(), done: g.status !== 'playing' }));
      }
    } catch (e) {}
  ...
  function resetGame() {
    ...
    setG(freshState()); setVal(''); setJustWon(false);
  }
  ...
  function shareUrl() {
    return `sourceoftruths.com/circa${isTodays ? '' : `?p=${PUZZLE.num}`}`;
  }
  function copyShare() {
    const text = playing
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f7f8fa', position: 'relative' }}>
      ...
      <div className="cc-wrap" style={{ position: 'relative', zIndex: 2, maxWidth: 1180, margin: '0 auto', padding: '18px 38px 80px', fontFamily: SANS }}>
        ...
        <div className="cc-mh" style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap', position: 'relative', paddingRight: 28, marginBottom: 16, borderBottom: '2px solid rgba(28,30,36,0.8)', paddingBottom: 11 }}>
          ...
          <div className="cc-ttl" style={{ display: 'flex', alignItems: 'baseline', gap: 9 }}>
            <h1 style={{ margin: 0, fontFamily: MONO, fontSize: 14, letterSpacing: '0.06em', fontWeight: 500, color: COLORS.ink }}>No. {PUZZLE.num}</h1>
            <span className="cc-ttl-dot" style={{ color: COLORS.faded }}>&middot;</span>
            <span className="cc-ttl-dt" style={{ fontFamily: SANS, fontStyle: 'italic', fontSize: 15, color: COLORS.faded }}>{PUZZLE.dateLabel}</span>
        ...
        {!playing && (
          <div style={{ background: '#fff', border: `2px solid ${COLORS.ink}`, borderRadius: 12, padding: '16px 16px 14px', marginBottom: 14 }}>
            <div style={{ fontSize: 19, fontWeight: 800, color: won ? COLORS.green : COLORS.rust, marginBottom: 4 }}>
              {Math.round((finalScore / 10) * 100)}% Complete
            </div>
            <div style={{ fontSize: 13.5, fontWeight: 700, color: COLORS.faded, marginBottom: 10 }}>
              {won
            ...
            {PUZZLE.sunday && (
              <div style={{ fontSize: 12.5, fontWeight: 600, color: COLORS.faded, fontStyle: 'italic', margin: '2px 0 8px' }}>The Sunday Edition — a trickier moment to place.</div>
            )}
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <button className="cc-btn" onClick={copyShare}><Share2 size={15} /> {copied ? 'Copied' : 'Share result'}</button>
              <button className="cc-btn" onClick={resetGame} style={{ borderColor: '#c3c8cf', color: COLORS.faded }}><RotateCcw size={15} /> Replay</button>
            </div>
            <DailyGamesPromo self="circa" refresh={g.status} />
        ...
        <div style={{ maxWidth: 620, margin: '20px auto 0' }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {[
              ...
              { n: myStats.played ? `${Math.round((myStats.perfect / myStats.played) * 100)}%` : '—', l: 'Dead On' },
        ...
        <div style={{ display: playing ? 'none' : 'block', maxWidth: 620, margin: '26px auto 0', background: '#fff', border: '1.5px solid rgba(20,22,28,0.12)', borderRadius: 12, padding: '14px 16px' }}>
          <QuizLeaderboard daily board={board} identity={identity} total={10} guessLabel="Guesses" />
        </div>
      </div>

      {/* the win moment: keepsake card */}
      {justWon && (
        <div onClick={() => setJustWon(false)} style={{ position: 'fixed', inset: 0, zIndex: 85, background: 'rgba(28,30,36,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, animation: 'ccfade .4s ease .3s backwards' }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: PAPER, border: `3px double ${COLORS.ink}`, borderRadius: 6, padding: '26px 30px 20px', maxWidth: 380, width: '100%', textAlign: 'center', fontFamily: SANS, boxShadow: '6px 6px 0 rgba(28,30,36,0.18)', animation: 'ccstamp .45s ease .3s backwards' }}>
            <div style={{ display: 'flex', gap: 3, justifyContent: 'center', marginBottom: 12 }}>
              {'CIRCA'.split('').map((ch, i) => (
                <span key={i} style={{ width: 24, height: 24, borderRadius: 3, background: i === 4 ? COLORS.accent : COLORS.ink, color: '#fff', fontFamily: SANS, fontWeight: 900, fontSize: 15, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', boxShadow: 'inset 0 1px 3px rgba(0,0,0,0.5)' }}>{ch}</span>
              ))}
            </div>
            <div style={{ fontFamily: SANS, fontWeight: 900, fontSize: 34, color: COLORS.ink, letterSpacing: '-0.01em', margin: '2px 0 6px', lineHeight: 1.15 }}>{exact ? 'Dead on.' : 'Circa!'}</div>
            <div style={{ fontFamily: MONO, fontSize: 12.5, color: COLORS.faded, marginBottom: 14 }}>No. {PUZZLE.num} &middot; {YEAR} &middot; {guesses.length} guess{guesses.length === 1 ? '' : 'es'} &middot; {elapsed}</div>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
              <button className="cc-btn" onClick={copyShare} style={{ background: COLORS.accent, color: '#fff', borderColor: COLORS.accent }}><Share2 size={15} /> {copied ? 'Copied' : 'Share result'}</button>
              <button className="cc-btn" onClick={() => setJustWon(false)}>See the moment</button>
            </div>
            <div style={{ fontFamily: MONO, fontSize: 10, letterSpacing: '0.1em', color: COLORS.faded, marginTop: 12 }}>sourceoftruths.com/circa</div>
          </div>
        </div>
      )}
