...
export default function LeaderboardSnippet({ board, identity, score, lastElapsed, fill = false, maxWidth = 320, margin = '0 auto 18px' }) {
  ...
  const row = (rank, name, sc, hot) => (
    <div key={`lb${rank}-${name}`} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 13px', borderTop: `1px solid ${C.faded}14`, background: hot ? T.accentSoft : 'transparent' }}>
  ...
  return (
    <div style={{ ...outer, background: T.white, border: `1px solid ${C.faded}33` }}>
