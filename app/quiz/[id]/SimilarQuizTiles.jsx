...
const GRADS = [
  ...
  `linear-gradient(135deg, #3f4756 0%, ${T.ink} 100%)`,
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 12 }}>
      {items.map((rq, i) => {
        ...
        return (
          <a key={rq.id} href={`/quiz/${rq.id}`} style={{ position: 'relative', display: 'block', textDecoration: 'none', borderRadius: 12, overflow: 'hidden', border: '1px solid rgba(20,22,28,0.30)', background: T.ink, aspectRatio: '16 / 10', boxShadow: '0 1px 2px rgba(20,22,28,0.06)' }}>
            ...
            <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, padding: '13px 14px' }}>
              ...
              <div style={{ fontFamily: FONT, fontSize: 16, fontWeight: 700, lineHeight: 1.14, color: T.white, textShadow: '0 1px 6px rgba(0,0,0,0.7)' }}>{rq.title}</div>
            </div>
          </a>
        );
      })}
