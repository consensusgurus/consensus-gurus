...
export default function TimedMcqClient({ quizId, mobile = false }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: COLORS.cream, color: COLORS.ink, position: 'relative', overflow: 'clip' }}>
      <div style={{ position: 'relative', zIndex: 3 }}><SiteHeader active="quizzes" inlay={<QuizPlayerBar />} /></div>
      <div style={{ position: 'relative', zIndex: 2, maxWidth: 1180, margin: '0 auto', padding: '4px 24px 80px' }}>
        ...
        {/* ── JOIN ── */}
        {tab === 'join' && (
          <JoinLeaderboardForm identity={identity} onJoined={(id) => { setIdentity(id); setMcqRevealed(true); setTab('play'); }} onViewLeaderboard={() => setTab('stats')} />
        )}
