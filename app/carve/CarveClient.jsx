...
// Carve — the daily equal-sum partition game.
//
...
export default function CarveClient({ puzzles = [], forceNum = null }) {
  ...
  function shareUrl() {
    return `sourceoftruths.com/carve${isTodays ? '' : `?p=${PUZZLE.num}`}`;
  }
  function copyShare() {
    const text = playing
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f7f8fa', position: 'relative' }}>
      ...
      <div className="cv-wrap" style={{ position: 'relative', zIndex: 2, maxWidth: 1180, margin: '0 auto', padding: '18px 38px 80px', fontFamily: SANS }}>
        ...
        <div className="cv-mh" style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap', position: 'relative', paddingRight: 28, marginBottom: 16, borderBottom: '2px solid rgba(28,30,36,0.8)', paddingBottom: 11 }}>
          ...
          <div className="cv-ttl" style={{ display: 'flex', alignItems: 'baseline', gap: 9 }}>
            <h1 style={{ margin: 0, fontFamily: MONO, fontSize: 14, letterSpacing: '0.06em', fontWeight: 500, color: COLORS.ink }}>No. {PUZZLE.num}</h1>
            <span className="cv-ttl-dot" style={{ color: COLORS.faded }}>&middot;</span>
            <span className="cv-ttl-dt" style={{ fontFamily: SANS, fontStyle: 'italic', fontSize: 15, color: COLORS.faded }}>{PUZZLE.dateLabel}</span>
            {PUZZLE.sunday && <span style={{ fontFamily: MONO, fontSize: 9.5, letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 500, color: '#fff', background: COLORS.accent, borderRadius: 4, padding: '2px 6px' }}>Sunday &middot; Big Board</span>}
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
            ...
            {identity && (g.t0 || errors > 0) && (
              <button onClick={() => { if (armReveal) { setArmReveal(false); revealEnd(); } else { setArmReveal(true); } }}
                ...
                <Eye size={13} /> {armReveal ? 'Tap again — ends the game and shows the carving' : 'Reveal & end'}
              </button>
            )}
          </div>
        )}
        ...
        {!playing && (
          <div style={{ background: '#fff', border: `2px solid ${COLORS.ink}`, borderRadius: 12, padding: '16px 16px 14px', marginBottom: 14 }}>
            ...
            {PUZZLE.sunday && (
              <div style={{ fontSize: 12.5, fontWeight: 600, color: COLORS.faded, fontStyle: 'italic', margin: '2px 0 8px' }}>The Sunday Edition — a bigger board with more blocks.</div>
            )}
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <button className="cv-btn" onClick={copyShare}><Share2 size={15} /> {copied ? 'Copied' : 'Share result'}</button>
              <button className="cv-btn" onClick={resetGame} style={{ borderColor: '#c3c8cf', color: COLORS.faded }}><RotateCcw size={15} /> Replay</button>
            </div>
            <DailyGamesPromo self="carve" refresh={g.status} />
